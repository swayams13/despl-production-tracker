...
describe("computeCpm — fitted lags reproduce the printed max envelope exactly", () => {
  ...
  it.each(pressureVesselProcesses.map((p) => ({ code: p.code, expected: p.envelopeFinishByMaxDays! })))(
    "P$code earlyFinish matches envelope.finishByMaxDays ($expected)",
    ({ code, expected }) => {
      expect(byId.get(code)!.earlyFinish).toBe(expected);
    },
  );
