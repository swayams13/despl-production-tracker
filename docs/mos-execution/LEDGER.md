...
| 2 Sep 2026 | **Unverified UI defect: `/my-day` card action buttons may still be 6px apart** (SPEC §8 assertion 3 requires ≥8px). The `test.fail()` in `supervisor-viewport.spec.ts` that recorded this was removed in S4a because it had become a hard failure (`Expected to fail, but passed`) — **not** because the defect was confirmed fixed. "File reason…" renders only for an overdue plan (`stage-sheet-launcher.tsx:619`) and no reachable data state on despl_test put the pair on screen, so it is unmeasurable, not proven gone. To settle it: create an overdue plan, load `/my-day` at 390x844, measure the gap between the card's action buttons. The e2e test will now fail honestly if it is still 6px. | [S4a] session | a UI item, whenever `/my-day` cards are next touched |
