...
| — | Rehearse the 20 migrations on a restored copy | — | ☐ | | `MERGE-RUNBOOK.md` §5. Roles **before** `pg_restore` (dump keeps ACLs/RLS, which name `despl_app`/`despl_web`). §5.5 is the **only** chance to verify the backfill against its source — after §5.7 `procurements` is gone forever. |
| — | Apply the 20 migrations to production, watched | — | ☐ | | §5.7. Abort point is immediately before it. Then restart the container (§5.8) so Prisma reconnects to the new schema. |
