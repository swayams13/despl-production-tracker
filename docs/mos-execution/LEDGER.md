...
| D3 | Four shared department accounts rotated | ◐ | 3 Sep | **Source half DONE:** `despl123@` deleted from `scripts/create-department-accounts.ts`; reads `DEPT_ACCOUNT_TEMP_PASSWORD` and **fails closed** if unset (proven: `Error: ... DEPT_ACCOUNT_TEMP_PASSWORD is not set`, thrown before any DB connection); `mustChangePassword` false → **true**; documented in `.env.example`. `git grep despl123 -- scripts/ src/ prisma/` → none (docs still cite it as an audit finding, correctly). typecheck + lint clean. **Live half PENDING — yours, no code needed:** Admin → Employees → **"Reset password"** on `fabrication@`, `qc@`, `production@`, `procurement@`. That calls the existing `resetUserPassword` (bumps `sessionVersion` killing live sessions, re-arms `mustChangePassword`, audits without logging the credential). The four accounts almost certainly exist live — the dashboard reads "ACTIVE USERS TODAY 0 of **7**". |
| D4 | `SEED_PASSWORD` confirmed set in Railway env | ☑ | 3 Sep | **ANSWER: it is NOT set.** The app service has only `AUTH_SECRET`, `DATABASE_URL`, `DIRECT_URL` + Railway's own vars (keys listed; no values printed). So `pnpm db:seed` — or `prisma migrate reset`, which auto-invokes the seed hook — against production would create every account with the public default `despl-dev-only`. Not currently exploited: production was seeded reference-only and no seed has run since. **Decide:** set `SEED_PASSWORD` in Railway, or (better, and the actual fix) make the demo seed refuse to run against production rather than rely on an env var nobody remembers. |
