...
| # | Question | Blocks |
|---|---|---|
| F-a | **Reconcile all 25 seeded routes against the 54 operations in the spec** — every step the floor performs must exist as a `RouteStep`, none silently folded into a neighbour. **DESPL-320's slice closed 26 Aug 2026** (its 11 components only touch the `PLATE` route): Rolling vs Forming resolved by citing the spec's own SHELL-01 note ("Team asked to track Rolling and Forming as two separate timed steps"), not a guess — `PLATE` now has both via a new `RouteTemplateVersion`. `EDGE_PREP`/`GRINDING`/`INSPECTION` turned out to already be separate `RouteStep`s on `PLATE`; their `GAP` flags were stale, not live gaps. The other 24 seeded routes (used by DE0463/DE0467, not DESPL-320) were **not** re-diffed this pass — still open. | Route definitions; changes the operation count |
