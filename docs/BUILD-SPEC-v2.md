...
| C26 | When a work-order stage is **both overdue and on an open hold point**, which single status should its spine segment / matrix cell show — **hold** or **overdue**? (Display precedence only; the hold-point count and the overdue count are computed independently at process grain and are unaffected — see DESIGN_SPEC §11.2/§11.4.) | Which colour SJ/management see first on the stage spine and Units×Stage matrix; nothing about gating or counts | **Hold wins** — a hold point is the actionable blocker (and blocks completion per invariant #4); overdue is the symptom. SJ to confirm he wants the blocker surfaced over the slippage. |
