...
- **Backup/DR needs actual numbers, not just "daily backups."** This is compliance-adjacent manufacturing data (QCP holds, MTC/heat-number traceability feeding client MDRs) — losing a shift's floor entries is a contractual risk, not an inconvenience. Target: **RPO ≤15 minutes** via continuous WAL archiving / point-in-time recovery (not just a daily `pg_dump`, which caps RPO at up to 24 hours) and **RTO ≤4 hours** during working hours. Development intentionally runs on Railway Hobby, where PITR isn't offered — this is fine, since PITR only needs to exist from go-live onward, not during development. **Confirmed plan:** upgrade the Railway workspace to Pro (+$15/mo minimum vs. Hobby, plus ~$1–5/mo WAL-archive storage — total hosting moves from PRD §9's ~$10–25/mo to roughly $25–45/mo) and enable PITR as part of Step 15, with enough lead time before go-live for the retention window to actually fill and for the connection-pool sizing in §2.5 to be re-verified against Pro-tier limits.
...
1. ~~**Backup/DR budget tradeoff** (§4)~~ — **resolved.** Development runs on Railway Hobby ($5/mo) by design; PITR is Pro-plan-only (Railway gates it behind Pro, opt-in from the Backups panel) and is not needed until go-live. The Hobby→Pro upgrade is a workspace billing change, not a service migration — same Postgres instance/volume/`DATABASE_URL`, no data movement, no app or schema change. Cost delta is the plan jump itself (Hobby $5/mo → Pro $20/mo, i.e. **+$15/mo minimum**) plus WAL-archive storage at bucket rate (~$0.015/GB-month, realistically $1–5/mo at this system's size) — pushing total hosting from PRD §9's ~$10–25/mo to roughly **$25–45/mo**. This upgrade + enabling PITR is scheduled as part of **Step 15** (see `docs/IMPLEMENTATION-GUIDE.md`), not before. Two sequencing gotchas folded into that step: PITR's restore window builds up prospectively (weekly full + daily incremental backups, ~4 weeks to fill) — enable it with enough lead time before the Step 15 restore drill that the drill tests a real window, not day-zero; and Hobby-tier `max_connections` is not representative of Pro-tier limits, so the connection-pool sizing check (§2.5) must be re-run after the Pro upgrade, not carried over from Hobby-era testing.
