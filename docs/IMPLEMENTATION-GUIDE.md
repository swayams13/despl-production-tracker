...
3. **Upgrade Railway workspace Hobby → Pro, then enable PITR** (continuous WAL archiving, RPO ≤15min — not just daily `pg_dump`), per `docs/ARCHITECTURE.md` §4. This is a billing/config change only — same Postgres instance/volume/`DATABASE_URL`, no app or schema change, no data migration. Do this with enough lead time before go-live that (a) the PITR retention window has actually filled (weekly full + daily incremental backups, ~4 weeks to reach full coverage — don't rely on day-zero coverage for item 2 below) and (b) the connection-pool sizing from §2.5 is re-verified against Pro-tier `max_connections`, since Hobby-tier limits (used during development) aren't representative.
