...
**Update, same day — real keyboard verification done**: a follow-up pass connected the Chrome extension and confirmed `clickableRowProps()` live, not just by code review — on `/departments/1`'s Open Items table, Tab landed the visible `:focus-visible` ring on a table row (proving it's really focusable), and Enter (no mouse) opened the `StageSheet` drawer. Also confirmed the fix on the QC hold-point `<span>` (the one that needed the truncation-style merge) and Reports' digest-history row, both via mouse click. Full account in `progress.md`'s "real browser verification" session. **Still not done**: a real screen-reader pass (VoiceOver/NVDA) — the keyboard-reachability half of this section is now verified live, the announcement/ARIA half is not.
