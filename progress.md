...
**Status:** 🟡 **Day 2 under way 13 Aug 2026 — `lib/schedule/` built, but BLOCKED on verification.** Foundation (tenancy/RLS/auth/RBAC), all seed data, and the scheduling engine now exist. **Still nothing calls the engine from a UI or persists a `ScheduleRun`** — no `lib/services/`, no visual component set, no department workspaces.

**⛔ BLOCKER — do this before any other work:** `pnpm install && pnpm test && pnpm lint && pnpm typecheck` has never actually been run against `lib/schedule/`. Two separate sessions confirmed this sandbox has no path to the npm registry at all (direct request to `registry.npmjs.org` → 403; the same request forced through the local agent proxy → still 403; no local registry mirror running either — see the verification caveat below for the full detail). **The next session — or the user, on a machine/environment with real registry access — must run the install+test+lint+typecheck sequence and fix anything that doesn't pass before `lib/schedule/` is trusted or built on top of.** Until that happens, treat `lib/schedule/` as *reviewed but unverified*, not *done*.
...
**`lib/schedule/` (IMPLEMENTATION-GUIDE Step 7) is now built: calendar, envelope (layer 1), CPM (layer 2, forward+backward+float), gating, feasibility, override — all in `src/lib/schedule/`, all pure functions over plain data, no Prisma import.** Read "What's done" / "What's remaining" below before touching code. `lib/services/` is still the next real gap: nothing persists a `ScheduleRun`/`ProcessPlan` row yet, and no UI calls the engine.
...
Confidence instead comes from (a) careful code reading, (b) an independent Python re-implementation of the forward/backward CPM pass and the calendar day-counting run against the real seed JSON (zero mismatches across all 36 processes, both directions), and (c) Python's stdlib `datetime` cross-checking every hardcoded date/weekday claim in the test files. That is real cross-validation, but it is **not** the same as a green `pnpm test` run — it cannot catch TypeScript-level mistakes (a wrong import path, a missed export, a type mismatch) since Python doesn't compile the actual `.ts` files. Do not assume the suite passes. This needs an environment with real npm registry access (the user's own machine, or a Claude Code environment configured with a more permissive network policy) — run `pnpm install && pnpm test && pnpm lint && pnpm typecheck` there and fix anything that doesn't match before trusting `lib/schedule/` further.
...
1. **Run `pnpm install && pnpm test && pnpm lint && pnpm typecheck` against `lib/schedule/`** — could not be done in this session's sandbox (no package-registry access at all). See the verification caveat above. Do this first, before anything else in this list.
2. **`lib/services/`** — still an empty README. No business-rule functions exist (start process, submit, verify, file delay reason) and nothing persists a `ScheduleRun`/`ProcessPlan` row — `lib/schedule/` computes plans but has no caller. Department workspaces and the process-update flow depend on this.
...
4. **One real department workspace** with gating/maker-checker/hold-point refusals actually firing (they exist in `lib/authz`/`lib/schedule/gating.ts` but nothing calls them against real process data yet).
