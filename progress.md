...
**Status:** 🟢 **Scheduling engine built and green (13 Aug 2026).** Foundation (tenancy/RLS/auth/RBAC), all seed data, 5 migrations — plus `lib/calendar/` and `lib/schedule/` (envelope, CPM, forward/backward/feasibility/override) with **75 passing tests**. The engine reproduces DESPL's printed 36-process envelope exactly (0 mismatches, terminal = 119 days). **Still nothing on screen** — no `lib/services/`, no UI beyond login + a read-only job list, no department workspaces. **Next: `lib/services/` (so schedules actually persist) + the visual component set.**
...
**Working on the `demo` branch. `lib/schedule/` is done; `lib/services/` is the next slice.**

---

## ▶ Resume point (read this first in a new session)

**The scheduling engine is built, tested and green. Nothing persists a schedule yet — that is `lib/services/`, the next slice.** Read this section, then "What's done" / "What's remaining" below before touching code.
...
### `lib/schedule/` — built 13 Aug 2026, and what the build actually proved

Five files under `src/lib/schedule/` (`types`, `graph`, `duration`, `envelope`, `cpm`) plus `src/lib/calendar/`. **Pure — no Prisma, no clock, no writes.** `lib/services/` will load rows, call in, and persist `ScheduleRun`/`ProcessPlan` with the audit row in one transaction. That split is why the engine is testable against hand-checked numbers instead of a seeded DB.

Four findings from building it, each now pinned by a test — **do not re-litigate these without re-reading the tests:**
...
2. **Never run CPM with min durations.** The lags were fitted to the max curve only. With min durations the terminal lands on **day 37** against an envelope that says 119. The optimistic curve is only ever meaningful as the envelope's own `finishByMinDays`. `resolveDurationDays()` therefore has no min path at all.
3. **Excluded processes must be spliced, never dropped.** PWHT (P21) sits mid-chain P20 → P21 → P22. Dropped naively, P22 loses its only predecessor and schedules on **day zero** — silently claiming NDE-after-PWHT can begin before the shell is welded. `bypassExcluded()` rewires predecessors to successors and composes lags. Related discovery: **P20/P21/P22 are fully concurrent in DESPL's table** (all three print start 86, finish 91, encoded as two −5 overlap lags), so the correct composed lag is −10 and P22 legitimately starts *before* P20 finishes. An assertion expecting finish-to-start here is asserting the wrong model — one was written and corrected.
4. **The DE0467 shortfall is 22, and it is convention-dependent.** Docs corrected (see below). It moves to **21** on an inclusive day-0 count, **14** against the late end of the dispatch window, and **38** against the revised order date. All four readings are computed in the tests so the choice stays visible.

**Docs corrected this session:** the "~26 working days" figure was wrong and is not reproducible from the dates. Now **22** in `BUILD-SPEC-v2.md` §1.5 (with the two counting conventions spelled out), `IMPLEMENTATION-GUIDE.md` Step 7, and the 11 Aug session-log row. Three new open questions raised for DESPL — **C21** (is a dispatch *window* a promise of its early or late date? worth 8 working days on DE0467 alone), **C22** (does a revised order date restart the clock?), **C23** (which processes are optional per client? PWHT is skippable but nothing is flagged `optional` in the lead-time table).

**Seed change:** `Job.deliveryDate` now takes the **earliest** date of a dispatch window, not the latest (`prisma/seed.ts`) — a feasibility warning must fire against the date first promised. Affects DE0467 only (25.10 → 15.10); DE0463's single date is unchanged. The raw range is still preserved verbatim in `Job.remarks`. **Needs `pnpm prisma migrate reset` to take effect in the DB** — the tests are pure and do not depend on it.
...
- **Scheduling engine** (13 Aug): `lib/calendar/` (working-day math, UTC-safe, day-0 convention pinned) + `lib/schedule/` (envelope, CPM forward/backward, feasibility, override-by-pin, excluded-process splicing, cycle refusal, `SCHEDULE_DATA_MISSING` for provisional/null-duration rows). 53 new tests.
- Full regression clean throughout: **75 unit tests** (22 authz + 21 calendar + 32 schedule), 6 e2e, lint, typecheck.

### What's remaining

1. ~~**`lib/schedule/`**~~ — **done 13 Aug 2026.** Note `ScheduleRun`/`ProcessPlan` still have **zero rows**: the engine computes, but nothing persists a schedule until `lib/services/` exists.
