...
**Explicitly out of scope, per the brief**: AUD-030 (the view's own separate `included`-handling defect) and AUD-065 (job intake's placeholder-only form labels) — neither touched. `lib/schedule/exclude.ts`'s `bypassExcluded` and every seeded `optional` value are untouched, per the brief's explicit constraints.
