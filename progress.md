...
**Next**: `20260908140000_aud030_excluded_process_stage_rollup` joins AUD-100/AUD-027/AUD-034's already-queued migrations for the same batched rehearse-then-apply-by-hand production window — not applied to production in this session, per this project's manual-migration discipline. This closes the last item named in the AUD-030 session brief; the `audit/SESSION-0N-AUD-*.md` series (Sessions 01–13) is now fully dispatched.
