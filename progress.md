...
**Status:** 🟡 **Day 2 under way 13 Aug 2026 — `lib/schedule/` and the visual component set both built, both BLOCKED on verification.** Foundation (tenancy/RLS/auth/RBAC), all seed data, the scheduling engine, and a 5-component visual set (KPI tile, progress ring, matrix heatmap, S-curve, milestone timeline) now exist. **Still nothing calls the engine from a real workflow or persists a `ScheduleRun`** — no `lib/services/`, no department workspaces; the new components are wired to a labeled sample-data preview page (`/component-gallery`), not live data.

**⛔ BLOCKER — do this before any other work:** `pnpm install && pnpm test && pnpm lint && pnpm typecheck` has never actually been run against ANY of today's code (`lib/schedule/` or the visual components). **Three** separate sessions now confirm this sandbox has no path to the npm registry at all (direct request to `registry.npmjs.org` → 403; the same request forced through the local agent proxy → still 403; no local registry mirror running either — see the verification caveat below for the full detail). This is a persistent, environment-level network policy, not a transient failure — retrying it again in-sandbox is not expected to help. **The next session — or the user, on a machine/environment with real registry access — must run the install+test+lint+typecheck sequence and fix anything that doesn't pass, and actually look at `/component-gallery` in a browser, before any of this is trusted or built on top of.** Until that happens, treat today's work as *reviewed but unverified*, not *done*.
...
**Git workflow, changed 13 Aug 2026:** new `demo` branch created from `main`. **Push to `demo` first; merge to `main` only after the user verifies and explicitly approves the promotion.** (This session ran on a harness-assigned branch, `claude/progress-status-check-8ttvkj`, per that session's explicit instructions — not `demo` — see its own session-log entry below for why.) **Exception, same day:** the user explicitly asked, in-conversation, for this branch's PR #1 to be merged straight to `main` (skipping `demo`) — done on that direct instruction, not on this session's own initiative. `demo` was not updated in the same action; reconcile it separately if that matters later.

---

## ▶ Resume point (read this first in a new session)

**`lib/schedule/` (IMPLEMENTATION-GUIDE Step 7) is now built: calendar, envelope (layer 1), CPM (layer 2, forward+backward+float), gating, feasibility, override — all in `src/lib/schedule/`, all pure functions over plain data, no Prisma import.** The visual component set is also now built (`src/components/viz/`) and previewable at `/component-gallery` against labeled sample data. Read "What's done" / "What's remaining" below before touching code. `lib/services/` is still the next real gap: nothing persists a `ScheduleRun`/`ProcessPlan` row yet, and no UI calls the engine or the components against real data.
...
**Model switched to Sonnet for Day 2 onward** (13 Aug 2026), per BUILD-SPEC-v2 §0 #9 / IMPLEMENTATION-GUIDE model discipline — architecture and spec decisions are locked, so Day 2+ is implementation against a written, approved plan.

**The spec's own DE0467 regression figure was wrong and has now been fixed, not bent around.** BUILD-SPEC-v2 §1.5 and IMPLEMENTATION-GUIDE Step 7 originally asserted a ~26 working-day shortfall; the correct arithmetic is 113 calendar days (24 Jun–15 Oct 2026) less 16 Sundays = 97 working days available, and 119 − 97 = **22**, not 26 (independent of the C1 working/calendar-days question logged below, which is a different what-if). Both docs are corrected to 22 (`docs/BUILD-SPEC-v2.md` §1.5 and its C1 row, `docs/IMPLEMENTATION-GUIDE.md` Step 7), and the engine's own regression test (`src/lib/schedule/feasibility.test.ts`) asserts 22, matching. The 22-day figure and every CPM/calendar number below were additionally cross-checked with an independent Python simulation (stdlib `datetime`, not the JS calendar module) against the real `seed/lead-time-model.json` — see the session-log entry for what was checked.

**Verification caveat, important for the next session:** this session's sandbox had no package-registry access at all, checked three separate ways: (1) `curl` direct to `registry.npmjs.org` → 403, (2) the same request forced through the local agent proxy (`--proxy http://127.0.0.1:38877`) → still 403, confirming this is not a proxy/no_proxy misconfiguration but the registry itself refusing the sandbox's network path, (3) checked for a local registry mirror (e.g. a Verdaccio instance) — none running. `pnpm install` itself fails even before reaching the project's own dependencies, because pnpm needs to self-fetch its own pinned version (`pnpm@9.15.9`) from the same blocked registry. So **`node_modules` could not be installed and `pnpm test`/`lint`/`typecheck` were never actually run — this is not fixable from inside the sandbox**, and is not something to keep retrying (per this environment's own proxy guidance: a 403 from a policy denial should be reported, not routed around).

Confidence for `lib/schedule/` instead comes from (a) careful code reading, (b) an independent Python re-implementation of the forward/backward CPM pass and the calendar day-counting run against the real seed JSON (zero mismatches across all 36 processes, both directions), and (c) Python's stdlib `datetime` cross-checking every hardcoded date/weekday claim in the test files. For the visual component set, the color tokens were mechanically diffed (hex-set comparison, not eyeballed) against the data-viz skill's own reference palette and validator output, and every file was re-read for TypeScript-shape issues (the dynamic-tag JSX pattern in `MatrixHeatmap` was caught and replaced with an explicit conditional for exactly this reason — a known TS/JSX rough edge that's easy to get subtly wrong without a compiler to check it). That is real cross-validation, but it is **not** the same as a green `pnpm test` run or actually looking at the page in a browser — it cannot catch TypeScript-level mistakes (a wrong import path, a missed export, a type mismatch) or visual/layout bugs. Do not assume the suite passes or that `/component-gallery` renders correctly. This needs an environment with real npm registry access (the user's own machine, or a Claude Code environment configured with a more permissive network policy) — run `pnpm install && pnpm test && pnpm lint && pnpm typecheck`, then `pnpm dev` and actually look at `/component-gallery` in both light and dark mode, and fix anything that doesn't match before trusting either piece of work further.
...
- Full regression clean through the 13 Aug data session: 22 unit tests, 6 e2e, lint, typecheck (last actually run before this session — see the verification caveat above for why this session's additions haven't been run the same way yet).
...
1. **Run `pnpm install && pnpm test && pnpm lint && pnpm typecheck`, then `pnpm dev` and look at `/component-gallery`** — could not be done in this session's sandbox (no package-registry access at all). See the verification caveat above. Do this first, before anything else in this list.
2. **`lib/services/`** — still an empty README. No business-rule functions exist (start process, submit, verify, file delay reason) and nothing persists a `ScheduleRun`/`ProcessPlan` row — `lib/schedule/` computes plans but has no caller, and the visual components have no real data to render. Department workspaces and the process-update flow depend on this.
3. **One real department workspace** with gating/maker-checker/hold-point refusals actually firing (they exist in `lib/authz`/`lib/schedule/gating.ts` but nothing calls them against real process data yet), built using the now-existing visual components against real data instead of the `/component-gallery` sample data.
4. **Management dashboard** and **client order view** (`/portal` content) — both read from data structures that don't exist until #2 is built.
5. **Deploy to Railway staging** — not started.
...
**⛔ `pnpm install`/`test`/`lint`/`typecheck` cannot run in the current sandbox** — see the top-of-file blocker banner. Needs a real npm-registry-connected environment before `lib/schedule/` or the visual component set can be trusted. Not a build-logic problem, an environment one.

Otherwise, twelve open questions (C1–C12 in BUILD-SPEC-v2 §7) each have a working default in place. **C1 — whether the lead-time table's "Days" are working or calendar days — is the highest-impact one**: it changes every computed date, and changes the DE0467 shortfall from 22 days to ~6 days.
