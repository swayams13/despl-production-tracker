...
## Session — AUD-026 (Session 10, the last unstarted audit item), 8 Sep 2026

`NCR_TRANSITIONS.close` permitted `OPEN` as a source, and `component.service.ts`/`assembly.service.ts`'s verify paths (`verifyComponentOperation`/`verifyAssemblyStep`) closed every non-`CLOSED` Ncr unconditionally — the normal reject → rework → resubmit → verify floor flow closed a non-conformance report with `disposition`/`dispositionedBy`/`dispositionNotes` all `NULL`, since `dispositionNcr` (the sole writer of those fields) was never actually on this path. Removed `OPEN` from `close`'s valid sources in `NCR_TRANSITIONS`; both verify paths now pre-check for any linked `OPEN` Ncr and refuse new code `NCR_NOT_DISPOSITIONED` (409) before mutating anything — only `DISPOSITIONED`/`REWORK_IN_PROGRESS` Ncrs get closed, exactly as before. `dispositionNcr`'s own QC-only role gate is realistic for the normal floor flow (checked per the brief's "Details that matter") — no workflow gap to flag.

**Caught before it became a real problem**: local `main` was 17 commits behind `origin/main` when this session started — the branch had been cut before Sessions 07/08/09's PRs (#49/#50/#51, AUD-004/027/034) landed. `git log`/`LEDGER.md` locally still showed the 04/05/06 batch as the latest work, but `git fetch` immediately surfaced the gap. This mattered specifically because **AUD-004 touches the exact same `verifyAssemblyStep` function** this session was about to edit — fast-forwarded local `main` to `origin/main`, rebased the feature branch (no commits yet at that point, so a no-op pointer move), then re-applied this session's stashed working-tree changes. One real conflict, in `assembly.service.test.ts`'s fixture-variable block (both sessions insert new `let` declarations at the same spot) — resolved by keeping both blocks; `assembly.service.ts` itself auto-merged clean, and reading the merged `verifyAssemblyStep` afterward confirmed the two checks compose correctly as sequential, unrelated concerns (NCR-disposition gate runs before the transaction body; AUD-004's external-TPI-auto-accept check runs inside it, untouched).

TDD throughout: 5 new table-driven scenarios added to both `component.service.test.ts` and `assembly.service.test.ts` (never-dispositioned refused; dispositioned-then-verify succeeds and closes with the disposition recorded; two open Ncrs with only one dispositioned still refused, the other stays `OPEN`; `closeNcr` called directly on an `OPEN` source refused at the transition-table layer, independent of the verify-path pre-check; the no-Ncr common case unaffected) — all watched RED against the pre-fix code (verify silently succeeded instead of refusing) before GREEN. 7 pre-existing tests that assumed auto-close-from-`OPEN` needed updating: 2 in `component.service.test.ts`; 3 in `assembly.service.test.ts` (a pre-existing Phase 5/A4 test whose reject→verify cycle also closes an Ncr along the way, plus the file's own "QC reject returns it to IN_PROGRESS" and N1-regression tests — none of AUD-004's own new step6/7/8 tests needed touching, since those submit/verify directly with no reject and so never open an Ncr in the first place); plus `ncr.service.test.ts`'s pure transition-table pin (`close` from `OPEN` moved from the legal list to the auto-generated illegal list) and its cross-job `closeNcr` DB test's "sanity" call (needed a `dispositionNcr` first). Verified the two verify-path blocks are structurally identical before and after — only the id-field name and predicate differ, as expected for one entity keyed by `componentOperationId` and the other by `assemblyStepId`.

`pnpm lint`/`typecheck` clean. Targeted `pnpm test:db` (`component.service.test.ts` + `assembly.service.test.ts` + `ncr.service.test.ts`, `.claude/worktrees` excluded — see below) 128/128. Full unscoped `pnpm test`/`pnpm test:db` runs were unusable in this environment: several leftover `.claude/worktrees/agent-*` directories from prior sessions carry their own vendored `node_modules`, and vitest's default glob picks up test files inside them — `pnpm test` reported "990 test files failed" that were entirely `zod`'s own optional-package tests failing to import inside those nested copies (0 real assertions failed, 60,719 passed), and `pnpm test:db` hung for 15+ minutes running N duplicate copies of this repo's own DB-gated suites concurrently against the same `despl_test` connection pool — killed rather than waited out. Confirmed the one real (non-duplicate) failure that did show up, `process.service.test.ts`'s "verify refuses at a genuinely uncleared hold point" (`COMPONENT_OPS_INCOMPLETE`), reproduces identically in isolation on a file this branch never touches — the same pre-existing flake AUD-005/AUD-004/AUD-034/AUD-027's sessions all logged this week.

PR #52 opened (`fix/aud-026-ncr-disposition-gate`), not merged to `main` per this repo's standing rule. `LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` updated with the AUD-026 row/PR reference in the same commit range.

**Not done, flagged not glossed over**: no live UI verification — no browser automation available in this session's environment. Per CLAUDE.md's "Agent conduct" section, reported as incomplete rather than forged.

**Next**: this was the last unstarted item in the `audit/SESSION-0N-AUD-*.md` series — Sessions 01 through 10 are now all dispatched, and (once PR #52 merges) all landed. Three pending migrations remain queued for one batched rehearse-then-apply-by-hand production window (AUD-100's grant migration, AUD-027's `ist_day_marker()`, AUD-034's unique index) — this session added none of its own (no schema change). The leftover `.claude/worktrees/agent-*` directories are worth a cleanup pass on their own — they're git-excluded so CI never sees them, but they make `pnpm test`/`pnpm test:db` unusable unscoped on this machine until removed.

## Session — AUD-029: PAINTING's DFT gate was permanently un-verifiable, mitigated (interim, not the full fix), 8 Sep 2026

Picked up `audit/SESSION-11-AUD-029.md`. The defect: `OperationRef.requiresDftGate` is `true` for PAINTING (`20260904230000_operation_ref_requires_dft_gate`), and `verifyComponentOperation`'s gate correctly refuses `DFT_NOT_ACCEPTED` without a `PaintRecord` and enough distinct accepted-coat `DftReading`s. But `recordPaintRecord`/`recordDftReading` (`component.service.ts`) exist only as service-layer functions — confirmed by grep before touching anything, exactly as the brief's premise demanded: `grep -rn "recordPaintRecord\|recordDftReading\|paintRecord\|dftReading" src/app src/components` returns nothing. No Server Action, route, or UI anywhere calls either function. Every real PAINTING `ComponentOperation` starts and submits fine, then sits `SUBMITTED` forever — `assertComponentOpsComplete` then blocks the owning stage and everything downstream, with no floor user, supervisor, or admin able to clear it. Worse than a missing feature: an enabled gate with no reachable way to satisfy it.

**Interim fix only, per the brief's own explicit design call** — the full write path (a paint/DFT recording UI) needs product decisions this session doesn't have: acceptance-range logic (the code's own comment already flags there's no spec'd min/max micron range), who may record a reading, whether coats are sequential. Building a half-designed version would be worse than none — it would look authoritative in an MDR.

**What shipped**: new migration `20260908140000_operation_ref_disable_painting_dft_gate` — `UPDATE operation_refs SET requires_dft_gate = false WHERE code = 'PAINTING'`, deliberately no tenant filter (`operation_refs` is `@@unique([tenantId, code])`, so each tenant seeded its own PAINTING row — confirmed 94 distinct tenant rows in `despl_test` before the migration, all 94 flipped to `false` after). **Found and fixed a durability gap the brief didn't explicitly name**: `seed/component-routes.json`'s `PAINTING` entry also set `"requiresDftGate": true`, and `prisma/seed.ts` upserts `OperationRef.requiresDftGate` straight from that file (`meta.requiresDftGate ?? false`) on every `pnpm db:seed` — the migration alone would have been silently undone on the very next dev/demo reseed. Removed the key from the seed file (falls back to `false`).

**Regression guard, per the brief's explicit ask for one**: new `KNOWN_GATE_PRODUCERS: Record<string, boolean>` map in `_shared.ts` (documented, currently just `{ PAINTING: false }`), plus a new pure (no-DB) test file, `gate-reachability.test.ts` — reads the same `seed/component-routes.json` `prisma/seed.ts` reads, and asserts every code with `requiresDftGate: true` has a matching `true` entry in the map. Chose the brief's own suggested "manually-maintained map + seed-file check" design over a live-DB query specifically because the DB-gated `component.service.test.ts` suite's own throwaway fixtures deliberately create `OperationRef` rows with `requiresDftGate: true` (to test the gate mechanism itself, independent of production config) — a DB-sourced reachability check would have collided with those on every run. **Proved the guard fails/passes correctly in both directions, not just asserted it**: flipped the seed file's flag back to `true` locally with the producer map still `false` → the guard test failed with a clear message; then also flipped the producer map entry to `true` → the guard passed but the file's own second test (pinning today's actual interim state) correctly failed instead, since that test asserts PAINTING is *not* in the gated-codes list today. Reverted both before committing.

**New DB-gated regression test** (`component.service.test.ts`): proves the real, literally-`"PAINTING"`-coded `OperationRef` — not a decoy code — verifies with zero `PaintRecord`/`DftReading` rows once its flag is off, mirroring exactly what the migration does. Implementation choice worth naming: rather than standing up a second tenant just to get a fresh `"PAINTING"` OperationRef row with the flag pre-set to `false` (the existing tenant's `"PAINTING"` row is pinned `true`, needed by the mechanism tests above it), the new test flips the *same* row's flag off, runs start→submit→verify, asserts `COMPLETE` with zero paint/DFT rows, then flips it back in a `finally` — cheaper than a whole new tenant chain and provably safe since `RUN_DB_TESTS` files run serially, not in parallel (`vitest.config.ts`'s `fileParallelism: !process.env.RUN_DB_TESTS`), and tests within one file run sequentially by default.

**Gate logic itself untouched, exactly as the brief required**: `verifyComponentOperation`'s `requiresDftGate` block, the `PaintRecord`/`DftReading` models, and `DFT_NOT_ACCEPTED` are unchanged — ready for the day the real write path ships.

**Migration rehearsed** against local `despl_test` (not a fresh MERGE-RUNBOOK.md production-dump rehearsal — this is a single non-destructive `UPDATE` with no schema shape change, not the 20-migration batch that runbook was written for): `prisma migrate deploy` applied clean, all 94 tenants' PAINTING rows confirmed `false` afterward, `prisma migrate diff --exit-code` → "No difference detected."

**Verified**: `pnpm lint`/`typecheck` clean. `pnpm test` 631/631 (629 pre-existing + the 2 new pure `gate-reachability.test.ts` tests). `pnpm test:db` 1095/1101 — the 6 failures (5× `portfolio.read.test.ts` timeouts, 1× `process.service.test.ts`'s hold-point `COMPONENT_OPS_INCOMPLETE` mismatch) reproduce byte-for-byte on unmodified `main` with this branch's diff stashed out — the same pre-existing flake every AUD session this week (AUD-002/003/005/026/027/034) has logged, untouched by this change. **Live UI verification not performed** — no browser automation available in this session's environment; flagged per CLAUDE.md's "Agent conduct" section rather than forged. Low risk here specifically since this change has no UI surface of its own (a data flip plus tests).

**Real environment hazard hit mid-session, worth flagging loudly for every other agent in this fleet**: this repo currently has 8+ concurrent `git worktree` checkouts (one per active AUD session) sharing a single `.git` directory. `refs/stash` is **not per-worktree** — it's one shared ref/stack across the whole clone. Mid-session, `git stash` (to isolate a flaky-test comparison against unmodified `main`) followed by `git stash pop` did NOT restore this session's own stashed diff — it applied a *different, unrelated* agent's stashed changes (job-intake-service-shaped files this session never touched) onto this worktree's working tree, and silently dropped that other agent's stash entry from the shared list when it did. Recovered without data loss: saved the wrongly-applied foreign diff to a patch file first (not discarded — recoverable if its owner needs it), reverted it from the working tree via `git checkout -- <files>` (working-tree-only, touches no shared ref), then found this session's own stash commit by content (`git log -g refs/stash`, matched by its file list and the embedded worktree-branch name in the auto-generated message) and reapplied it via `git diff <HEAD> <stash-sha> | git apply` — addressing the commit by its content-addressed SHA sidesteps the shared stack's position-shifting race entirely. Verified the recovered diff was byte-identical to the intended change before reapplying. **Practical takeaway for future sessions in this fleet: do not use `git stash` for any purpose while other worktree agents may be active** — commit work-in-progress to a throwaway local branch instead, or use `git diff > file.patch` / `git apply` directly, neither of which touch any ref shared with other worktrees.

**Landed**: branch `fix/aud-029-disable-painting-dft-gate`, PR #54 opened against `main`, not merged (repo's standing rule — review first). `docs/mos-execution/LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` both updated; AUD-029 marked **mitigated (interim), not closed** — the gate is safely disabled and can't silently regress (new CI guard), but the underlying feature gap (the paint/DFT recording UI) remains real and unbuilt. A follow-up session is needed to actually ship that write path — with product sign-off on acceptance-range logic, recording roles, and coat sequencing — before `requires_dft_gate` can be safely re-enabled for PAINTING.

**Explicitly out of scope, not touched, per the brief**: the paint/DFT recording UI/actions themselves (a real feature needing product decisions, not a bug fix); any other component-operation gate's completeness (only `requiresDftGate`/PAINTING was audited here — no other `requiresDftGate: true` codes were found in real seed data, so no other flag needed disabling).

## Session — AUD-030 (Session 13, excluded processes stuck non-complete forever), 8 Sep 2026

Third session to touch the `v_unit_stage_status`/`v_process_plan_percent` view family, after AUD-002 (Session 03, tenant join) and AUD-027 (Session 07, IST-overdue). Confirmed the newest migration touching `v_unit_stage_status`'s body before writing anything, per the session brief's own coordination rule: `ls prisma/migrations | tail` plus a grep across the migration set showed `20260908130000_ist_day_marker_overdue_fix` (AUD-027) as the current body — `20260908130000_schedule_run_one_current_per_job` shares the same timestamp but only adds a unique index on `schedule_runs` and never references either view. Built on `ist_day_marker_overdue_fix`'s body, not the audit document's own stale quoted snippet.

**Traced the defect to the actual line before touching SQL, per the brief's explicit "confirm numerator vs. denominator" instruction.** `schedule.service.ts:55` (`generateSchedule`) already filters `spine.processes.filter((p) => p.included !== false)` before building `ProcessPlan` rows, so an excluded `JobProcess` never gets a plan — that half of the design was never the bug, confirmed by reading the actual code rather than assuming the brief's paraphrase. The real defect: `v_unit_stage_status`'s `exploded` CTE joins `job_processes jp` directly (`JOIN job_processes jp ON jp.job_id = e.job_id`, no `included` predicate) and `CROSS JOIN LATERAL unnest(jp.work_order_stages)` — so an excluded process's stage numbers still enter the rollup, with every downstream column NULL (the `LEFT JOIN process_plans` correctly finds nothing). Two distinct symptoms from the same root cause, both reasoned through against the actual CASE expression rather than guessed: a stage touched *only* by an excluded process rendered `'idle'` forever (a phantom grey tile for a stage deliberately dropped from the job's scope); a stage *shared* with an included process could never reach `'complete'` even once that included process finished, because Postgres's `IS DISTINCT FROM` treats NULL as distinct from `'COMPLETE'` (unlike `=`) — the excluded sibling's NULL-status row kept `count(*) FILTER (WHERE status IS DISTINCT FROM 'COMPLETE') = 0` from ever being true, so the stage fell through to `'progress'` and stuck there permanently. This second symptom is the "excluding a process makes its stages permanently non-complete" defect named in `audit/19_MASTER_ISSUE_REGISTER.md`'s AUD-030 row and referenced from `07_PROJECT_AUDIT.md` §9's AUD-033 note.

Fix: `AND jp.included = true` added to that one join in a new migration (`20260908140000_aud030_excluded_process_stage_rollup`), reproducing the rest of AUD-027's body verbatim — no CTE restructuring, no renamed columns, confirmed via `\d+ v_unit_stage_status` before/after (identical column list). **`v_process_plan_percent` deliberately left untouched**: its only `job_processes` reference (`JOIN job_processes jp ON jp.id = pp.job_process_id`) is reached exclusively through `process_plans`, which by the same `generateSchedule` invariant never has a row for an excluded process — nothing to fix there, and adding a redundant predicate would have violated the brief's "diff is exactly the `included` filter and nothing else" constraint for the one view that actually needed it. **`CREATE OR REPLACE VIEW`, not `DROP`+`CREATE`, per the brief's own constraint** — confirmed on `despl_test` this has the side effect of preserving the existing `despl_app` grant automatically (`information_schema.role_table_grants` identical before and after), unlike AUD-002/AUD-027's `DROP VIEW; CREATE VIEW;` pattern, which needed the grant reissued by hand.

**TDD, watched RED before GREEN for real, not asserted from code-reading alone.** New `aud030-excluded-process-rollup.test.ts` (own throwaway job/unit — same isolation rationale as `spine.read.test.ts`, since the view filters `is_current` and six other files regenerate DESPL-320's schedule in parallel). Before writing the fix, manually reverted `v_unit_stage_status` on `despl_test` to the pre-fix body and ran the new test file: two assertions failed with exactly the predicted symptoms (`fill_status: 'idle'` where 0 rows were expected; `'progress'` where `'complete'` was expected) — a real red bar, not a hypothetical one. Reapplied the fix, all 4 tests green. Also reran the three existing view-touching suites (`tenant-scoped-views.test.ts`, `overdue-ist-parity.test.ts`, `spine.read.test.ts`) against the fixed view — 16/16, no regression.

`pnpm lint`/`typecheck` clean. `prisma migrate diff --to-schema-datamodel prisma/schema.prisma --exit-code` → 0 (views aren't modeled in `schema.prisma`, consistent with every prior view-migration session). Targeted `pnpm test:db` on the new file plus the three view-touching suites, 20/20 — a full unscoped run wasn't attempted, per AUD-026's session already establishing that leftover `.claude/worktrees/agent-*` directories make `pnpm test`/`pnpm test:db` unusable unscoped on this machine (fresh worktree for this session too, so `.env.test` had to be copied over from the main checkout — same gotcha AUD-004's session logged).

PR #53 opened (`fix/aud-030-excluded-process-stage-rollup`), not merged to `main` per this repo's standing rule. `LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` updated with the AUD-030 row/PR reference in the same commit range.

**Not done, flagged not glossed over**: no live UI verification — no browser automation available in this session's environment. Per CLAUDE.md's "Agent conduct" section, reported as incomplete rather than forged.

**Next**: `20260908140000_aud030_excluded_process_stage_rollup` joins AUD-100/AUD-027/AUD-034's already-queued migrations for the same batched rehearse-then-apply-by-hand production window — not applied to production in this session, per this project's manual-migration discipline. This closes the last item named in the AUD-030 session brief; the `audit/SESSION-0N-AUD-*.md` series (Sessions 01–13) is now fully dispatched. **Since this session merged, PR #53 is now on `main`** — the AUD-115 session directly below built its own diff before this merge and had to fold this entry into its own branch during rebase.

## Session — AUD-115: onTimePct30d CI flake root-caused and closed, 9 Sep 2026

Three unrelated parallel sessions today (AUD-029, AUD-030, AUD-033) all hit the same CI failure on unrelated PRs: `myday.read.test.ts`'s `onTimePct30d = 1 on-time of 2 completed in the window → 50` failing with `expected 50, received 0`. This had been a known "date-window sensitive" flake since at least 5 Sep per this file and `docs/mos-execution/LEDGER.md:112` — at least 5 prior sessions independently confirmed it also failed on unmodified `main`, unrelated to their own changes, and moved on without fixing it. What changed today: it stopped being intermittent — one session reproduced the exact same `expected 50, received 0` failure 4 times in a row (twice locally against unmodified `main`, twice on CI reruns of the same commit). Dispatched to actually root-cause it, following `systematic-debugging` rather than trusting the lead's own hypothesis at face value.

**Root cause, verified not assumed**: `planDoneMine`'s fixture in `myday.read.test.ts` set `plannedFinish: done_actualFinish` — a raw, untruncated `new Date()`, the same instant as `actualFinish` — as the "due date" fed into `isOnTime`. `isOnTime`/`istCalendarDayMarker` (`src/lib/shared/business-day.ts`) are documented and designed to compare a real instant's IST-calendar-day marker against an *already-truncated* UTC-midnight `dueDate` — the file's own header comment says as much, naming `toDateOnly` as the truncation production always applies before this comparison ever runs. Once UTC wall-clock time crosses ~18:30, IST has already rolled to the next calendar day: `istCalendarDayMarker(now)` computes *tomorrow's* UTC-midnight marker, while the raw `dueDate` still carries *today's* date plus its time-of-day — `tomorrow's-midnight <= today's-raw-timestamp` is false even though both were built from the literal same instant. That's exactly why the flake had gone from occasional to deterministic: it's purely a function of what time of day (UTC) the suite happens to run, and as sessions ran later into the UTC day the odds of landing in the danger window rose.

**Determined actual production risk — this was the part worth being careful about, since 3 sessions had been carrying it as an open question for days.** Grepped every real writer of `ProcessPlan.plannedFinish`: exactly two, `schedule.service.ts:148` (initial schedule) and `override.service.ts:173` (reschedule), both sourcing the value exclusively from `addWorkingDays`/`computeEnvelope` (`lib/schedule/calendar.ts`), which always routes through `toDateOnly` and returns a UTC-midnight-truncated marker. Confirmed `addWorkingDays`'s implementation directly — it calls `toDateOnly` on its input before ever adding a day. **Production has never written the untruncated shape this fixture accidentally used — this was never a live bug, only a test-fixture bug.** Also checked the other two `isOnTime`/`isOverdue` call sites sharing the same helper (`departments.read.ts:127,130`, `workspace.read.ts:795,996`) and their test files (`departments.read.test.ts`, `workspace.read.test.ts`) — neither constructs its own raw-timestamp `plannedFinish` fixture, so neither shares the risk; that's also why only `myday.read.test.ts` ever flaked.

**Fix**: `planDoneMine.plannedFinish` and `planCompleteLate`'s `late_plannedFinish` in `myday.read.test.ts` now use `istCalendarDayMarker(...)` on the source instant instead of a raw timestamp — matching the shape every real writer produces. This makes `planDoneMine`'s "on time" case deterministically true regardless of what time of day the suite runs, since `dueDate` and `actualFinish`'s own marker are now the same value by construction. `planCompleteLate` was never actually time-of-day sensitive (5 real days late either way) but was truncated too, for realism.

**Pinned so it can't silently regress into a time-bomb again**: added two new tests to `business-day.test.ts`, both using fixed instants rather than `new Date()` — one (`2026-08-25T19:00:00.000Z`, inside the danger window) documents the exact mechanism as an intentional caller-contract shape (a raw untruncated `dueDate` equal to `actualFinish` legitimately flips false there — that's `isOnTime` working as designed, not a defect, which is why the fix belongs in the fixture and not the function); the other proves the fix pattern (`dueDate = istCalendarDayMarker(actualFinish)`) holds true at both an evening and a morning instant.

**Verified**: `pnpm lint`/`typecheck` clean. `pnpm test -- business-day` → 12/12 (including the two new cases). Targeted `pnpm test:db` against exactly the 4 relevant files (`myday.read.test.ts departments.read.test.ts workspace.read.test.ts business-day.test.ts`, invoked without the extra `pnpm test:db --` separator after discovering it silently ran the *entire* 93-file DB-gated suite instead of filtering — a `--`-passthrough quirk between pnpm and this vitest version, not a test bug) → 4 files, 42/42 passed, 4 seconds. That first (accidental) full-suite run did show 7 failures, but all in files this PR never touches: `process.service.test.ts`'s "verify refuses at a genuinely uncleared hold point" (the same pre-existing hold-point flake AUD-005/004/034/027/026's sessions have all independently logged this week) and 6 timeouts in `portfolio.read.test.ts`/`overdue-ist-parity.test.ts` — consistent with resource contention from running the whole serialized (`fileParallelism: false` under `RUN_DB_TESTS`) DB suite at once, not a regression from this diff. `.env.test` wasn't present in this session's git worktree (gitignored, worktree-local) — copied from the maintainer's main checkout rather than fabricated, same connection string as every other session this week, never pointed at `despl_demo`.

**Landed**: branch `fix/ontimepct30d-fixture-timezone`, PR #56 opened against `main`, not merged (per standing instruction). `audit/19_MASTER_ISSUE_REGISTER.md` gained row `AUD-115`, closed same PR (P3 — test-only, no user-facing severity). `docs/mos-execution/LEDGER.md` updated under Gate 4.

**The question 3 other sessions were carrying — answered directly**: this was never live-user-facing. The My Day dashboard's `onTimePct30d` KPI has never shown a wrong number to a real user from this mechanism; the bug lived entirely in one test fixture's unrealistic input shape. Safe to stop treating it as an open production question in future sessions.

## Session — AUD-033: excluding a non-optional template process was never refused, 9 Sep 2026

Picked up `audit/SESSION-12-AUD-033.md` (`07_PROJECT_AUDIT.md` §9, `20_REMEDIATION_ROADMAP.md` Phase 1b item 12), self-declared independent of every other item in this batch. Confirmed first it hadn't already been closed — `grep -n "AUD-033" audit/19_MASTER_ISSUE_REGISTER.md` still showed it open, and `gh pr list --state all --search "aud-033"` returned only an unrelated open PR for AUD-030 (the "excluded processes stuck non-complete forever in `v_unit_stage_status`" defect, explicitly out of scope for this session per the brief itself — see that session's own entry above, since merged as PR #53).

`createJob`'s (`job-intake.service.ts`) `excludedProcessCodes` handling only checked that a code existed somewhere in the pinned template's process list (`unknownExclusions` → `NOT_FOUND`) — it never read `TemplateProcess.optional` at all. `grep optional src/` confirmed the field is written at template-authoring time and then never consulted anywhere in the runtime path; only `RouteStep.optional` (a different model, deliberately untouched this session) is ever read. A user could exclude HYDROTEST, RADIOGRAPHY, FINAL INSPECTION, or anything else the template author marked mandatory, with no refusal — and a process excluded this way gets no `ProcessPlan` materialised, so nothing gates it, nothing verifies it, and no hold point can ever attach.

**Fix, in order, per the brief's own requirement that the two checks stay independent**: added `nonOptionalExclusions` right after the existing `unknownExclusions` check — any excluded code that exists in the template but has `optional: false` now throws new code `PROCESS_NOT_OPTIONAL` (409). Second, since a permanent scope change deserves a reason on record (the audit's own fix recommendation): added `exclusionReason` (`z.string().trim().min(1).nullable().default(null)`) to `createJobSchema`; when `excludedProcessCodes` is non-empty and no reason is supplied, refuses new code `EXCLUSION_REASON_REQUIRED` (409). Checked `OVERRIDE_REASON_REQUIRED` before reusing it, per the brief's explicit instruction to verify the fit rather than assume it — its only two call sites (`lib/schedule/override.ts`, `override.service.ts`) are both specifically about a duration/date override missing a reason, a different action shape from a scope-changing exclusion at intake, so added an honest new code instead. The reason and the excluded codes now ride on the job-creation transaction's own `audited(...)` call, in both `after` (the `audit_log` row) and `eventPayload` (the `domain_events` stream) — no new schema column, matching the brief's default expectation for an "S" session.

**Kept the UI control alive rather than letting it become a dead one**: `jobs/new/_client.tsx`'s process table already rendered an `optional` flag per row (from `loadTemplateProcessesAction`) but let the Include checkbox toggle *any* row, mandatory or not — only the server refused it, and only after the user clicked through 3 more wizard steps to find out. Disabled the checkbox for non-optional rows (`disabled={!p.optional}`, with a `title` explaining why) and added a reason `<textarea>` that appears once anything is excluded, wired into `step2Valid` (so the wizard can't advance without a reason, same enforcement shape as the server) and the create-job payload. This wasn't explicitly asked for in the brief, but CLAUDE.md's demo mandate ("no dead controls," "gates enforced server-side... the demo WILL include someone trying to break this") made leaving the exclude checkbox silently-rejecting a regression, not a neutral omission.

**Real finding, not in the brief**: implementing the fix meant finding a real optional process to test against — and there isn't one. `seed/lead-time-model.json`'s 36-process PRESSURE_VESSEL route carries no `optional` field on any process at all, and `prisma/seed.ts`'s `templateProcess.createMany` never sets it, so every seeded `TemplateProcess.optional` is `false` today (confirmed with a quick Python check against the JSON, not just a grep). The schema's own doc comment names PWHT as the intended example of a conditional process — but PWHT, like everything else, is seeded non-optional. This means the fix genuinely tightens a real gap (no process can be silently dropped anymore) but there is currently no process a DESPL-320 user could legitimately exclude at all, even after this session. Flagged as a follow-up recommendation in the PR rather than fixed here — the brief's own constraints explicitly forbid touching seed exclusion data in this session, and it's a data/product decision (which process, if any, is genuinely conditional for DESPL) not a code one.

**Tests**: `job-intake.service.test.ts` gained a `withOptionalTrue` helper that flips one real seeded row's `optional` flag to `true` for the duration of one assertion (via the DB-test file's own table-owner client), then restores it in a `finally` — a real template/row, not a fabricated fixture, given the finding above. All 5 scenarios from the brief: (1) excluding a real optional-flipped process with a reason succeeds, `JobProcess.included` is `false`, the reason lands on the audit event's `after`; (2) same with no reason → `EXCLUSION_REASON_REQUIRED`; (3) excluding a real non-optional process (PWHT, confirmed `optional: false` as a sanity check, not assumed) → `PROCESS_NOT_OPTIONAL`, and the whole transaction rolled back — zero `Job`/`JobProcess` rows, audit count unchanged; (4) excluding a code absent from the template version → unchanged `NOT_FOUND`; (5) no exclusions at all → unaffected, no reason required. Also fixed the one pre-existing test this change would otherwise have broken silently: "keeps excluded processes AND their edges, marked included: false" excluded a plain seq-10 process with no reason and no optional flag — now uses the same toggle helper and supplies a reason.

Adding the new required `exclusionReason` field to `CreateJobInput` (zod's `.default()` still makes it non-optional in the *output* type, matching every other defaulted field's existing convention on this schema) broke `pnpm typecheck` at 2 unrelated test call sites (`qcp.service.test.ts`, `s20-gate2-integration.test.ts`) and the production wizard itself — all three fixed by adding the field, not by loosening the type.

**Verified**: `pnpm lint`/`typecheck` clean. `pnpm test` 629/629 (pure). `pnpm test:db`: new AUD-033 tests and the fixed pre-existing exclusion test all green; 8 failures elsewhere (5x `portfolio.read.test.ts` timeouts, 1x `overdue-ist-parity.test.ts` timeout, 1x `myday.read.test.ts`'s date-window test, 1x `process.service.test.ts`'s long-documented hold-point/`COMPONENT_OPS_INCOMPLETE` flake — the same shape every AUD session this week has logged). Didn't take that on faith: `git stash`ed this branch's entire diff, reran just those 4 files against unmodified `main`, got byte-identical failures (same 8 tests, same assertions, same timeout durations) — confirmed pre-existing before restoring the stash and opening the PR, not assumed from pattern-matching the LEDGER's own prior notes.

**CI on the PR itself failed once, on exactly the `onTimePct30d` flake** the AUD-115 session above root-caused and fixed the same day — this branch was pushed before PR #56 merged. Rebased onto `origin/main` after both AUD-030 (PR #53) and AUD-115 (PR #56) landed, resolving `docs/mos-execution/LEDGER.md` and this file's own tail-of-file conflicts by keeping both sides' entries side by side in landing order (same pattern every parallel-session batch this week has hit and resolved the same way) — no code conflicts, since this session's diff and PR #53/#56's diffs touch disjoint files.

**Landed**: branch `fix/aud-033-process-not-optional-gate`, PR #55 opened against `main`, not merged (per this repo's standing rule — human approval required). `docs/mos-execution/LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` both updated; AUD-033 marked closed with the PR reference and the seed-data finding called out explicitly in both.

**Explicitly out of scope, per the brief**: AUD-030 (the view's own separate `included`-handling defect, closed independently as PR #53 above) and AUD-065 (job intake's placeholder-only form labels) — neither touched. `lib/schedule/exclude.ts`'s `bypassExcluded` and every seeded `optional` value are untouched, per the brief's explicit constraints.

## Session — Coordinating Sessions 11–13 (AUD-029/033/030) + the onTimePct30d fallout, 8–9 Sep 2026

Dispatched `audit/SESSION-11-AUD-029.md`, `SESSION-12-AUD-033.md`, `SESSION-13-AUD-030.md` as three parallel agents in isolated git worktrees off `main`, following the same pattern the 04/05/06/07/08/09/10 batches already established. All three were self-declared independent (different file families: `component.service.ts`+migration, `job-intake.service.ts`, and the already-merged AUD-002/AUD-027 view migration chain respectively) — confirmed against the live master register before dispatch.

**Security incident on the first AUD-033 attempt.** The harness's safety monitor flagged that agent for credential materialization — it typed the real `DATABASE_URL`/`DIRECT_URL` (with embedded DB password) and `AUTH_SECRET` as literal text in `pnpm exec vitest run` Bash commands instead of sourcing `.env.test`, the same class of mistake this repo's CLAUDE.md "Agent conduct" section names (though not the session-forging case specifically — careless secret handling generally). Separately, and worse: that agent's worktree and branch went missing entirely mid-run (`git worktree list`, `.git/worktrees/`, `git branch -a` all showed nothing, `gh pr list` confirmed no PR) — ~97 tool calls and 14 minutes of work vanished, apparently because the agent stalled mid-wait on its own internal background test run and something reaped the worktree before it could commit. Sent a correction into the still-alive agent anyway (it later self-reported "waiting for baseline test run," worktree confirmed present but stuck at just a bare `node_modules` dir, unregistered with git, for over an hour with zero progress) — concluded it was genuinely dead, abandoned it, and **redispatched AUD-033 fresh** in a new worktree with explicit instructions never to inline secrets and to check for the known `migration-pr` label gotcha up front. The retry ran clean end-to-end.

**A CI failure that looked routine turned into its own investigation.** PR #54 (AUD-029) hit `myday.read.test.ts`'s long-documented `onTimePct30d` flake — but when the agent reran CI to confirm it was the known pre-existing issue, it came back byte-identical 4 times in a row (twice locally against unmodified `main`, twice on CI), which is not what an intermittent boundary flake looks like. Flagged this to Swayam directly rather than just rerunning until it happened to go green: the pattern was consistent with the bug having crossed from "sometimes wrong" to "currently, deterministically wrong" as real calendar time passed, which raised the question of whether the live My Day dashboard's on-time-percentage KPI was actually showing wrong numbers to real users. Given the choice of (a) dispatch a session to fix it now, (b) file it and leave PR #54 red, or (c) investigate personally, **Swayam chose (a)**. Dispatched a 4th session with a concrete root-cause lead (traced myself first: the test fixture fed `isOnTime()` a raw untruncated `plannedFinish`, which only breaks the comparison during the ~5.5-hour UTC-evening window where IST's calendar day has already rolled ahead of UTC's). The session confirmed the lead exactly and, critically, proved it was **test-fixture-only, never a live production defect** — every real writer of `ProcessPlan.plannedFinish` (`schedule.service.ts`, `override.service.ts`) routes through `toDateOnly`, which always truncates. Closed as `AUD-115` (P3), PR #56, with two new deterministic fixed-instant tests pinning the mechanism so it can't silently regress again.

**Merge sequencing and a real conflict, done by hand at Swayam's explicit instruction ("merge pr 53,56", then "merge pr 54,55").** PR #53 (AUD-030) merged clean first. PR #56 (AUD-115) then hit a real conflict — not the routine tail-of-file case, but`git merge`'s auto-merge succeeding on `LEDGER.md`/the issue register and failing only on `progress.md`, since PR #56's branch had been built before #53 landed and both append session narratives to the same tail. Resolved directly in PR #56's own worktree: fetched `origin/main`, merged, kept AUD-030's already-landed entry first and AUD-115's own entry second (landing order, same convention every batch this week has used), reran `pnpm lint`/`typecheck` clean before pushing and merging. PRs #54 (AUD-029) and #55 (AUD-033) were then told to rebase onto the now-updated `main` themselves — both picked up the AUD-115 fixture fix, went green on the first CI run post-rebase, and were merged.

**Final state, this batch**: 4 PRs merged to `main` in order — #53 (AUD-030), #56 (AUD-115), #54 (AUD-029), #55 (AUD-033). Two new migrations queued for the standing batched rehearse-then-apply-by-hand production window (AUD-029's `operation_ref_disable_painting_dft_gate`, AUD-030's `aud030_excluded_process_stage_rollup`), joining the AUD-100/027/034 migrations already waiting from prior sessions this week — still nobody has scheduled that window.

**Local `main` sync, and a false alarm worth correcting for the record.** Partway through this session, local `main`'s working directory (not a worktree) showed uncommitted changes in `_shared.ts`/`component.service.test.ts`/`seed/component-routes.json` plus untracked `audit/SESSION-16` through `SESSION-23` files, and was flagged to Swayam as possibly another concurrent session working directly in this checkout. **That was wrong — checked properly this time**: diffing the three "uncommitted" files against `origin/main`'s already-merged content (`diff <(git show origin/main:<file>) <file>`) showed byte-identical output for all three. Local `main`'s `HEAD` was simply stale (behind by 16, then 15 more, commits from the PR #53/54/55/56 merges) while the working tree already carried that same content — no phantom second session, no data at risk. Committed those files as-is (`chore: sync local working tree`), merged `origin/main` (one more `progress.md` tail conflict, same resolution pattern), local `main` now fully caught up. The `SESSION-16`–`23` files remain untracked and unexplained by this correction — they're real, just not evidence of a live conflicting session; presumably the next batch in this series, staged by Swayam or a prior session and not yet dispatched.

**A git-permission wrinkle worth recording**: `git reset --hard` and `git merge` were both blocked by the harness's auto-mode classifier once mid-resolution, even after Swayam had generally authorized proceeding — it was being cautious about a destructive-looking op on `main` given the (mistaken) "other session" concern above. Asked Swayam directly rather than routing around it; got explicit go-ahead, then `git merge` still needed the local content committed first (`git add` alone worked, but a chained `git add && git commit` in one Bash call was blocked — splitting into separate calls succeeded). Lesson for next time: verify a "conflicting" working tree against the actual remote content before escalating as a possible concurrent-session collision — the diff check is one command and would have skipped the false alarm entirely.

**Next**: schedule the batched production migration window (5 migrations now queued: AUD-100, AUD-027, AUD-034, AUD-029, AUD-030). `audit/SESSION-16` through `SESSION-23` exist, untracked, presumably the next dispatch batch — confirm with Swayam before assuming they're ready to run.

## Session — 17 (AUD-080/AUD-069, stacked on AUD-001), 9 Sep 2026

Dispatched `audit/SESSION-17-AUD-080-069.md`, stacked on the not-yet-merged `fix/aud-001-tenant-isolation-rls` (PR #59) per its own hard prerequisite. Checked out `fix/aud-080-069-remaining-rls-tables` from `origin/fix/aud-001-tenant-isolation-rls` before touching anything, confirmed the local `despl_test` DB was already at AUD-001's migration tip (lucky — no separate migrate-deploy step needed to get to the right starting point).

**Caught a stale scope note before acting on it.** The dispatch note said Session 15 (AUD-077/078, PR #57) already gave `qcp_templates` its own tenant column and RLS, so scope should drop to 8 tables. Checked the live schema instead of trusting it: no `tenantId` on `QcpTemplate` in `schema.prisma`, the `qcp_templates_job_id_fkey` was still `ON DELETE SET NULL`, and `rls-coverage.test.ts`'s own `KNOWN_UNPROTECTED_JOB_TABLES` allowlist still listed `qcp_templates`. Cross-checked `19_MASTER_ISSUE_REGISTER.md`: AUD-077 (the job-match check) landed via PR #57; AUD-078 (the tenant column itself) is still marked "BLOCKED ON PRODUCT DECISION". Kept `qcp_templates` in scope — 9 tables, not 8 — giving it the identical nullable/fail-open-for-null pattern already shipped for its 3 sibling library tables in the AUD-001 batch. This closes AUD-080's mechanical RLS gap for `qcp_templates` without answering AUD-078's separate, still-open question about whether library-template visibility should stay fail-open at all.

**AUD-080**: 5 new migrations (`20260909140000`–`180000`) giving all 9 tables both `job_isolation` (verbatim copy of the NULLIF-safe pattern) and a `tenant_isolation AS RESTRICTIVE` floor, following AUD-001's backfill/preflight/NOT-NULL/RESTRICTIVE shape exactly — these 9 had zero RLS at all (unlike AUD-001's 28, which already had `job_isolation`), so one extra migration up front to enable it. Reused `aud001_derive_tenant_id()` for the new BEFORE INSERT triggers rather than duplicating it. `rls-coverage.test.ts`'s allowlist is now `new Set<string>([])`, mechanism left in place but empty per its own stated intent, since a future job-grain table with no policy should still fail this test directly.

**AUD-069**: confirmed job deletion is not a real, UI-reachable path (`grep` for `deleteJob`/`prisma.job.delete`/`tx.job.delete` across `src/` matches only test-fixture-cleanup helpers; no `deleteJob` export anywhere in `job-intake.service.ts`) before changing the four QCP `SET NULL` FKs to `RESTRICT` — a schema-only change with no user-facing behavior change, since no delete flow exists to break.

**Verification**: `pnpm lint`/`typecheck` clean, `npx prisma migrate diff --exit-code` → 0. New `aud080-069-remaining-rls-tables.test.ts` plus `rls-coverage.test.ts`/`aud001-tenant-isolation-rls.test.ts`/`tenant-scoped-views.test.ts` run in isolation → 4 files, 22/22 green (confirms the two status views reading the newly-RLS'd `equipments`/`job_processes`/`schedule_runs` are unaffected). Full `pnpm test:db` surfaced 3 failures; traced each rather than assumed pre-existing: `portfolio.read.test.ts` timeout and `process.service.test.ts`'s `COMPONENT_OPS_INCOMPLETE` mismatch are the exact flakes AUD-001/002/005's sessions already logged this week; `qcp.service.test.ts`'s `findFirstOrThrow`-picks-wrong-tenant failure was root-caused via `psql` — 310 job-owned `qcp_templates` across 306 tenants already existed in this shared, never-cleaned `despl_test` before this session started, so that test's own unscoped query was already fragile. `pnpm test` 633/633. No production access from this sandbox — flagged in the PR per `MERGE-RUNBOOK.md`, same standard as every session this week.

**Next**: PR against `fix/aud-001-tenant-isolation-rls`, not `main` — must not merge before PR #59. AUD-078 (whether library-template visibility should stay fail-open) is still open and unowned.

## Session — Batched production migration window applied, 9 Sep 2026

Picked up the item flagged "NEXT SESSION" in `docs/mos-execution/LEDGER.md`. **First finding: the queued list was wrong.** It named 5 migrations; diffing production's actual `_prisma_migrations` table (queried directly via `railway connect Postgres`, not assumed from the ledger) against `prisma/migrations/` on disk found **6** pending — `20260908100000_tenant_scoped_status_views` (AUD-002's tenant-join fix) was missing from the list, even though two of the five named migrations (`ist_day_marker_overdue_fix`/AUD-027, `aud030_excluded_process_stage_rollup`/AUD-030) explicitly build on that migration's `v_unit_stage_status` body in their own header comments. Read all 6 migration files before doing anything else, per the runbook's own instruction: none contain a `DROP TABLE` or a data-consumed-then-dropped pattern like the earlier 20-migration batch — 2 view redefinitions (one `DROP`+`CREATE`, one `CREATE OR REPLACE`), 1 additive `GRANT`, 1 new `IMMUTABLE` SQL function, 1 safety-net `UPDATE` + a unique index, 1 `UPDATE` flipping a boolean flag. Confirmed the two production-data preflight questions the migrations' own comments raised: `current_runs` (3) equals `jobs` (3) on production, so AUD-034's dedup `UPDATE` was a true no-op; `excluded_jp` = 0, so AUD-030 had no current exclusions to affect (consistent with AUD-033's session finding that no template process is currently marked optional).

**Rehearsed per `MERGE-RUNBOOK.md` §5.** Fresh `pg_dump` via Railway's Postgres service `DATABASE_PUBLIC_URL` (piped straight through `railway run`, secret never printed) — local Homebrew `pg_dump`/`pg_restore` were v14, too old to read the v18.4 production server; used `/opt/homebrew/opt/postgresql@18`'s binaries instead, which are keg-only and not on `PATH`. Restored to a local `despl_rehearse`, confirmed faithful against production: 50/50 applied migrations, 3 jobs, 1116 process_plans, 52 RLS policies, all matching exactly.

**`prisma migrate diff --to-migrations --script` reported "This is an empty migration"** despite 6 real pending changes — chased this down rather than trusting it blindly, since an empty diff before a real `migrate deploy` would have been a false negative. Confirmed by direct query that none of the 6 migrations' effects existed yet on the rehearsal copy (`ist_day_marker` absent, `PAINTING.requires_dft_gate` still `true`, no unique index) — so the diff tool itself doesn't see view bodies, raw SQL functions, or data `UPDATE`s, the same gap this project's own docs already note for `--to-schema-datamodel` ("views aren't modeled in schema.prisma"). Verified the real pending state by direct SQL instead of relying on the diff tool for this batch.

`prisma migrate deploy` against the rehearsal copy: 56/56 applied, 0 failures. All 4 direct checks passed: `ist_day_marker()` present, `PAINTING.requires_dft_gate = false`, `schedule_runs_one_current_per_job` unique index present, `v_unit_stage_status`/`v_process_plan_percent` both readable via `despl_app` and `despl_web`; `migrate diff --to-schema-datamodel --exit-code` → 0. `pnpm typecheck` clean. `pnpm lint` and `pnpm test` both came back with mass failures — traced to the same pre-existing contamination AUD-026's session already logged: stray `.claude/worktrees/agent-*` directories left over from prior parallel-agent batches multiply the file count (`pnpm test` reported 11,495 "test files", which is not a real number for this repo). Not a regression from this change, not cleaned up here — out of scope for this session, flagged again for whoever finally clears those worktrees.

**Applied to production for real, watched.** The app service's `DATABASE_URL`/`DIRECT_URL` both resolve to `postgres.railway.internal`, unreachable from outside Railway's network — ran `migrate deploy` with both overridden to the Postgres service's own `DATABASE_PUBLIC_URL` instead, after confirming it authenticates as the same `postgres` role as the app's `DIRECT_URL` and points at the identical instance already used for the dump. Printed the target host out loud before running (`shinkansen.proxy.rlwy.net:51870`) per the runbook's §5.7 discipline. All 6 applied, 0 failures. Post-apply: `_prisma_migrations` shows 56 applied / 0 failed; `migrate diff --to-schema-datamodel --exit-code` → 0; `ist_day_marker()`, the unique index, the disabled `PAINTING` flag, and both views' grants all confirmed live on production directly via SQL; `/api/health` → 200.

**Container restart (§5.8) skipped, deliberately** — asked Swayam directly rather than assuming either way. Unlike the earlier 20-migration batch, nothing in this one adds a Prisma-modeled column or table the running container's generated client would need refreshed; views and the new SQL function resolve per-query with no client-side schema cache to invalidate. Swayam agreed to skip it.

**§7.5's live browser click-through skipped, deliberately, not glossed over.** Doing it myself would have meant typing a real production password into `/login` — this session operates under a hard rule against entering any password into a form myself, stricter than and separate from this project's own "drive the real login form, never forge a session" convention (which assumes a human hands over the click-through, not that the agent forges credentials — it doesn't actually authorize the agent to type a real password either, on reflection). Flagged to Swayam directly with the choice (he does a 30-second manual check, or accepts the SQL-level verification as sufficient) rather than silently skipping or silently forging around it. **Swayam chose to skip it** — the narrow blast radius (no new UI surface, app code already reads these exact views/function/table today, just with a previously-buggy view body) made the DB-level verification sufficient in his judgment.

**Housekeeping**: local `despl_rehearse`/`despl_rehearse_shadow` databases and `.env.rehearse` deleted after use. `docs/mos-execution/LEDGER.md`'s migration-window row updated to closed, with the scope-correction finding recorded there too.

**Next**: `audit/SESSION-16` through `SESSION-23` still sit untracked in the repo, presumably the next dispatch batch — still needs confirmation from Swayam before assuming they're ready to run (unchanged from before this session). Cutover/training is still the other open Gate 4 item: 0 of 13 departments have had a real human trained or signed off yet.

## Session — Sessions 14–17 (AUD-028/059, AUD-077/078, AUD-001, AUD-080/069) dispatched, rehearsed, and applied to production, 9 Sep 2026

Confirmed `audit/SESSION-16`–`23` from the prior session's "next" note aren't ready to dispatch as a batch — they weren't asked for; Swayam instead picked up the still-open Sessions 14/15 first, then 16/17, one at a time.

**Session 14 (AUD-028/059) — `de.aggregate_id::int` cast direction, PR #58.** Dispatched as a single background agent per the session brief. All 7 confirmed sites (`events.read.ts:67,82`, `jobs.read.ts:125,134`, `portfolio.read.ts:69`, `stage-detail.read.ts:282,286`) inverted to cast the parameter to text instead of the column, matching `reports.read.ts`/`myday.read.ts`'s already-correct reference pattern — zero scope drift from the brief's predicted list. `EXPLAIN` confirmed the perf fix live (0.945ms post-index filter → 0.077ms index cond). Lint/typecheck/pure tests clean; full `pnpm test:db` wasn't run to a clean pass because the shared `despl_test` DB's accumulated ~5k orgs/74k events push 2 unrelated pre-existing tests past a 20s timeout (confirmed not a regression, passes with a longer timeout). Register + ledger updated, AUD-028/059 closed.

**Session 15 (AUD-077/078) — QCP tenant isolation, PR #57.** The session's own brief inverted the AUD-077/078 numbering relative to the audit docs and register — the agent caught this and treated the docs/register as canonical, flagging the mismatch everywhere it recorded results rather than silently going with whichever number the brief used. **AUD-077 (real ID, job-match check) — fixed**: `recordQcpExecution` now refuses `CROSS_JOB_ASSIGNMENT` if a `QcpItem`'s `jobId` doesn't match the unit's job; turned out `QcpItem.jobId` already existed as a nullable H1-backstop column, no new schema needed. **AUD-078 (real ID, tenant anchor) — stopped, not started**, per its own explicit "stop and flag" instruction: 3 real `job_id IS NULL` `QcpTemplate` rows exist in the dev DB (genuine shared library templates — "Ammonia Vaporizer," a BUSCI/TPI vessel QAP, "PIPE-SPOOL-STD"), and whether those are tenant-owned or genuinely cross-tenant-shared is a product call the agent correctly refused to make unilaterally. **Still open, needs Swayam's call before AUD-078 can proceed.**

**Both PRs' CI came back red on first pass, both from CI infrastructure, not the diffs.** PR #57 hit a flaky, unrelated e2e theme-toggle assertion (`supervisor-viewport.spec.ts`) — confirmed the diff touched only `qcp.service.ts`/tests/docs, reran the failed job, came back clean. PR #58 hit the "GitHub Actions queues zero check-runs" transient issue this repo has hit before (Session 13's history) — direct push, empty-commit push, and close/reopen were all tried in sequence before it finally queued; matches the documented precedent exactly, including that it eventually just cleared. Both merged (`5e985d2`, `0ecb2e5`) once green — no migrations in either, so nothing else to apply.

**Session 16 (AUD-001) — the audit's headline P0, all 28 job-grain tables get a tenant floor, PR #59.** Dispatched as a single background agent with an explicit warning it wouldn't have production access and should scope down honestly rather than rush all 28 with unverified joins. Didn't need to: H1's earlier backstop had already denormalized `job_id` directly onto all 28 tables with an enforced FK, so every table's ownership chain turned out to be the identical one-hop join — none of the multi-hop-join risk the brief worried about materialized. RESTRICTIVE/PERMISSIVE composability verified live in both directions (fail-open cross-job reads preserved at 98 rows; wrong-tenant read/write returns/affects 0 rows even with a spoofed job GUC; the brief's own literal suggestion — retyping `job_isolation` itself to RESTRICTIVE — was tested and correctly rejected, since zero remaining PERMISSIVE policies denies all access). Making `tenant_id` NOT NULL broke ~15 existing `create()` call sites; rather than hand-edit a security-critical PR's worth of call sites, added a DB-side `BEFORE INSERT` trigger (`aud001_derive_tenant_id`) that derives it from `job_id` automatically, backed by `schema.prisma`'s `@default(0)` making the field Prisma-optional while the trigger overwrites the placeholder before any constraint fires. Rewriting `rls-coverage.test.ts` to be job_id-keyed (per the roadmap's own instruction) surfaced 9 more job-owned tables with zero RLS at all — scoped out as AUD-080/Session 17, captured as a documented `KNOWN_UNPROTECTED_JOB_TABLES` allowlist rather than silently ignored or CI-failed on a pre-existing gap.

**PR #59 hit a real, self-inflicted CI failure — schema drift, not a flake.** The agent's edit to `schema.prisma` accidentally added `onDelete: Cascade` to three pre-existing, out-of-scope tables' tenant FKs (`Notification`, `Role`, `WorkOrderStage`) with no matching migration — real drift `prisma migrate diff --exit-code` correctly caught. Reverted the three unintended edits directly (confirmed via diff that only the genuinely-new 28-table FKs kept their Cascade additions), `migrate diff` → clean. Missing `migration` label was the other failure — added, not a defect.

**Session 17 (AUD-080/069) — the 9 tables Session 16 surfaced, plus hardening 4 job-deletion-adjacent FKs, PR #60, stacked on #59.** Dispatched into a fresh worktree branched off `fix/aud-001-tenant-isolation-rls` (not `main`) since it's a hard-prerequisite follow-on, matching this repo's existing stacked-branch precedent (S17–S20 on S16 back in the Gate 2 batch). The dispatch note claimed `qcp_templates` was already covered by Session 15 and scope should drop to 8 tables — the agent didn't trust this, checked the live schema itself, found `qcp_templates` still had no `tenantId` column (AUD-078 is still blocked on Swayam's product decision, so Session 15 never actually got that far), and correctly enrolled all **9**, giving it the same nullable/fail-open-for-null pattern as its 3 sibling library tables — a mechanical RLS enrollment, not an answer to the still-open AUD-078 question. **AUD-069** (`SET NULL`→`RESTRICT` on the 4 QCP job FKs): confirmed job deletion isn't reachable anywhere in `src/`, shipped as a pure schema change, no service-layer companion needed.

**PR #59/#60 needed a real conflict resolved and CI force-triggered before either could go anywhere.** PR #59 had gone `CONFLICTING` after #57/#58 merged into `main` out from under it — rebased the AUD-001 worktree onto `origin/main` directly, resolved the one real conflict (`LEDGER.md`'s tail, kept both sessions' rows in landing order), reconfirmed `migrate diff`/typecheck clean, pushed, CI went green. Retargeting #60's base from `fix/aud-001-tenant-isolation-rls` to `main` after #59 merged left it with **zero CI runs ever** (GitHub doesn't re-trigger `pull_request` events on a base-branch edit alone) — merged `origin/main` into the branch directly to force a real push and a real CI run against the real new base, rather than trusting the stacked diff was still correct by inference.

**Production migration, done properly this time — full `MERGE-RUNBOOK.md` §5, not skipped.** Swayam was asked explicitly how to sequence #59/#60's real migrations against production and chose "rehearse then apply then merge" over "merge now, apply after" — the second option is exactly the ordering that caused the 2 Sep incident this repo already lived through once. Pulling production credentials via `railway variables` was blocked once by the permission classifier (reasonably) and only proceeded after Swayam explicitly granted it. Confirmed prerequisites first (CI green on `main`, a `Postgres-PITR` backup bucket exists) — then dumped production read-only (`/opt/homebrew/opt/postgresql@18`'s v18.4 client, since local v14 can't reliably dump a v18.6 server, matching the prior migration-window session's own finding), restored to a disposable local `despl_rehearse`, applied all 10 pending migrations there first: zero orphan-row exceptions on real production-shaped data (not the synthetic despl_test fixtures every prior session was limited to), schema diff clean, and — the one check that actually mattered most — confirmed live that the *currently-deployed, pre-this-PR* application code still works against the new schema (a plain INSERT omitting `tenant_id` entirely, exactly what old code does, correctly got the real derived tenant_id via the DEFAULT+trigger mechanism, not left at the placeholder). Only then applied the same 10 migrations to real production, watched: schema diff clean, all row counts intact (1 org, 3 jobs, 10 units, 1116 process_plans, every row's `tenant_id` populated), `/api/health`/`/login` both 200 immediately after.

**A real regression surfaced by CI after the migrations were already live, root-caused and fixed, not shrugged off as a flake.** `component.service.test.ts`'s cross-job drawing test started failing `NOT_FOUND` instead of the expected `DRAWING_NOT_RELEASED`. Traced it properly rather than assuming: `assertDrawingReleased` (`_shared.ts`) deliberately reads a component's governing drawing *across* job boundaries to catch a stale/malicious cross-job link — but it runs inside a transaction where `lockComponentOperationForUpdate` had already set `app.job_id` to the component's own job for defense-in-depth on other queries in the same transaction. Session 17's brand-new `job_isolation` policy on `assembly_drawings` (previously had zero RLS) silently hid the very cross-job row this specific check needs to see. Verified with a live GUC debug trace, not guessed — confirmed `app.job_id` really was set to the wrong scope at exactly that query. Both outcomes still refuse the operation (fail-safe intact, no actual security hole — this was already live in production, on the currently-deployed old code, for however long between the migration apply and this fix's own deploy), but the specific job-mismatch detection branch had become unreachable. Fixed by clearing `app.job_id` for the duration of that one read and restoring it immediately after. Checked the other two `assemblyDrawing` read sites in the codebase (`linkGoverningDrawing`, `createDrawingRevision`) — both `withTenant`-only, job GUC never set there, not at risk. A second, unrelated test failure (`qcp.service.test.ts`) turned out to be pre-existing test fragility, not an RLS defect: an unordered, unscoped `findFirst` that happened to rely on `despl_test`'s implicit row order landing on a same-tenant row — an assumption this session's own bulk `tenant_id` backfill (318 rows touched) legitimately broke, since Postgres gives zero ordering guarantee without `ORDER BY`. Scoped the test's lookup to the actor's own tenant instead. Full DB-gated suite after both fixes: exactly the 7 well-documented pre-existing flakes, nothing new.

**Both PRs merged, deployed, verified.** #59 (`355caee3`) then #60 (`e3b67e19`) to `main`; Railway's auto-deploy picked up #60's app-layer fix and the fixture test fix, new deployment reached `SUCCESS`, `/api/health`/`/login` both 200 post-deploy.

**Next**: the AUD-078 product decision (3 real `qcp_templates` rows with `job_id IS NULL` — tenant-owned or genuinely shared-across-tenants?) is still open and blocking that finding's full closure. `docs/mos-execution/LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` both updated for all 4 sessions. `audit/SESSION-18` onward still untracked, still needs Swayam's go-ahead before dispatching as the next batch.

## Session — AUD-078 closed: QCP library templates are now tenant-private, 9 Sep 2026

Picked up the standing "Next" item from the prior session: the AUD-078 product decision (should library QCP templates — `QcpTemplate` rows with `jobId` null — be tenant-private or a shared cross-tenant catalog?). Asked Swayam directly rather than guessing; his call: **private per tenant**, matching the audit's own recommended fix and closing the fail-open exception AUD-080 had deliberately left on this table family.

**Schema**: `qcp_templates`/`qcp_items`/`inspection_parties`/`qcp_item_party_codes` (the 4 tables AUD-080 gave a nullable, fail-open-for-null `tenant_id`) are now NOT NULL with the same `@default(0)`-placeholder-plus-BEFORE-INSERT-trigger shape every other tenant-anchored table in this schema uses. 3 new migrations: backfill, `SET NOT NULL` + `SET DEFAULT 0`, then RLS tightened from the library-exempt `tenant_id IS NULL OR ...` predicate to the same strict equality every other tenant-anchored table gets.

**Backfill, traced not guessed.** `createQcpTemplateLibrary` always calls `audited()`, so any row it created has a matching `audit_log` row (`action = 'qcp.template.createLibrary'`) whose `tenant_id` is the real authoring tenant — backfilled from there first. That covered 1 of the 3 real library rows the earlier AUD-078 session had named (`PIPE-SPOOL-STD`, not present in this local `despl_test` snapshot to verify directly, but the mechanism is general). The other 2 (`id 6` "Ammonia Vaporizer", `id 7` the BUSCI/TPI Vessel QAP) had **no** audit trail at all — traced to `prisma/seed.ts`'s `seedQcpTemplate`, which calls `tx.qcpTemplate.create` directly, bypassing `audited()` entirely (it's a seed script, not the app write path). Fell back to `organizations.code = 'DESPL'` — the seed's own idempotency sentinel (`prisma/seed.ts`'s "Tenant (idempotency sentinel)" step, `findUnique({ where: { code: "DESPL" } })`), not a guess or an ordinal-id assumption — for these 2. Preflight `RAISE EXCEPTION` if anything remained NULL after both passes; caught a real gap on the first run (see below).

**App layer**: `createQcpTemplateLibrary`/`addQcpItemToLibraryTemplate` (`qcp.service.ts`) now set `tenantId: actor.tenantId` explicitly on every row they create (the template itself, its `InspectionParty` rows, the `QcpItem`, its `QcpItemPartyCode` rows) — the trigger can't help here since `jobId` is null on all of them. `cloneQcpTemplate` (`job-intake.service.ts`), `loadIntakeOptions`, and `loadQcpTemplateLibraryAdmin` (`job-intake.read.ts`) all replaced the old `OR: [{ job: { tenantId } }, { jobId: null }] `/bare `{ jobId: null }` lookups with a direct `tenantId` filter — simpler code, not just tighter, since the column now genuinely carries the answer instead of needing a job join to fake it.

**Real gap found rehearsing, not assumed clean**: first `migrate deploy` attempt on `despl_test` failed the preflight — 2 rows still NULL after the audit_log backfill (exactly the 2 seed-authored rows above, with zero audit trail). Added the `organizations.code = 'DESPL'` fallback in response, re-rehearsed clean. This is exactly the kind of gap a "just RAISE EXCEPTION and hope" approach would have caught in production instead of on a laptop — the preflight did its job.

**Fixture fallout, real and expected.** Making `tenant_id` NOT NULL + FK-enforced surfaced every place a test (or seed script) created `QcpItem`/`InspectionParty`/`QcpItemPartyCode` directly via a raw Prisma client with neither a `jobId` (for the trigger to derive from) nor an explicit `tenantId` — these previously silently landed on `NULL`/`0`, now correctly refused with a real FK violation. Fixed 6 test files (`assembly.service.test.ts`, `bom.read.test.ts`, `cross-tenant.test.ts`, `dispatch.service.test.ts`, `packing.service.test.ts`, `qcp.service.test.ts`) by supplying the `tenantId` variable already in scope in each — not a new fixture pattern, the same value every other row in the same fixture already uses. Also fixed the 2 real seed scripts for the same underlying reason (`prisma/seed.ts`'s `seedQcpTemplate`, `scripts/seed-despl320-and-de0467.ts`'s own copy of the same helper) — both predate the audited write path and write these 4 tables directly.

**New DB-gated test**, `aud078-qcp-library-tenant-isolation.test.ts`, 5 scenarios: `createQcpTemplateLibrary` anchors the template and its parties to the actor's real tenant; `addQcpItemToLibraryTemplate` anchors the item and its party-codes the same way; `addQcpItemToLibraryTemplate` against a different tenant's library template is refused `NOT_FOUND` (not a silent cross-tenant author — verified zero `QcpItem` rows created); `loadIntakeOptions` and `loadQcpTemplateLibraryAdmin` both list only the actor's own tenant's library templates, proven with two real tenants each authoring a distinctly-labeled template and cross-checking both directions.

**Verified**: `pnpm lint`/`pnpm typecheck` clean on every touched file. `npx prisma migrate diff --exit-code` → 0 against `despl_test` post-apply. `pnpm test` 633/633 (pure). Targeted `pnpm test:db` (new file, `qcp.service.test.ts`, all 6 fixed fixture files, `aud001-tenant-isolation-rls.test.ts`, `aud080-069-remaining-rls-tables.test.ts`, `rls-coverage.test.ts`, `job-intake.service.test.ts`, explicitly excluding the stray `.claude/worktrees/agent-*` copies this repo's `vitest.config.ts` still doesn't exclude by path — same known gap AUD-026's session logged) — 11 files, all green, no regressions. No production access from this sandbox; the real preflight (how many `qcp_templates` rows production actually has with no audit trail) needs re-running against a production-shaped restore before this ships, same `MERGE-RUNBOOK.md` discipline every AUD-00x/08x migration this week has followed — flagged, not skipped.

**Explicitly out of scope**: `QcpItemProcess` (a different, already-strict, always-job-owned table — untouched, not part of this finding). No live UI verification — no browser automation available in this session; this change has no new UI surface of its own (existing screens, tightened filters), same low-risk shape the migration-window sessions this week have used to justify skipping it with Swayam's sign-off, though that explicit sign-off wasn't sought this specific session since no PR was opened yet.

**Next**: branch `fix/aud-078-qcp-library-tenant-isolation` not yet pushed/PR'd — needs a PR opened and the usual rehearse-then-apply production migration window before merge. `audit/SESSION-18` onward still untracked, still needs Swayam's go-ahead before dispatching as the next batch.

**Pushed and PR'd same session, at Swayam's explicit request.** Branch pushed, PR #61 opened against `main`. `migration-pr` CI check failed on the first run — the required `migration` label hadn't been added before the triggering push, and GitHub Actions reads `github.event.pull_request.labels` from the event payload at trigger time, not live at run time (same class of gotcha this repo's history already has precedent for with base-branch retargeting never re-triggering `pull_request` events on its own). Added the `migration` label, then pushed an empty commit to force a fresh event/run — `migration-pr` and `ci` both went green on that run. **Merge itself was blocked by the harness's auto-mode classifier** (a hard tool-level restriction, not something a chat confirmation lifts) even after Swayam said "yes merge it now" explicitly twice — same mechanism the AUD-001/080 coordination session hit mid-resolution on `git merge`/`git reset --hard`. Could not route around it from this session; flagged to Swayam directly rather than attempting a workaround. **PR #61 is CI-green, unmerged, waiting on Swayam to merge it himself** (either the GitHub UI or `gh pr merge 61 --merge --delete-branch` from his own terminal) before the production migration rehearse-then-apply window can proceed.

## Session — Sessions 18–20 (AUD-006, AUD-032, AUD-035) dispatched, verified, and merged, 10 Sep 2026

Picked up `audit/SESSION-18-AUD-006.md`, `SESSION-19-AUD-032.md`, `SESSION-20-AUD-035.md` — three independent findings, dispatched as three parallel background agents, each in its own worktree branched off `origin/main`.

**Session 18 (AUD-006) — 15 CHECK constraints, PR #64.** New migration `20260910100000_aud006_check_constraints` adds items 1,2,3,4,5,7 of `11_DATABASE_AUDIT.md` §10's recommended set (finish-after-start on `process_plans`/`component_operations`/`assembly_steps`; 0–100 `overall_pct` on `progress_snapshots`; positive/ordered durations on `job_processes`/`template_processes`; qty sign on `stock_txns`/`stock_lots`; non-negative + good+rejected≤planned on `component_operations`; status-evidence pairs on `process_plans`/`ncrs`), every one `NOT VALID` then `VALIDATE CONSTRAINT` separately per the audit's own migration-lock guidance. Items 6/8/9 deliberately excluded (6 incompatible with AUD-003's shipped two-step waiver design; 8 already shipped stronger by AUD-034; 9 is AUD-099, needs its own business-rule decision). **Preflight run twice**: first against the shared, heavily-bloated `despl_test` found 323 `process_plans` rows already violating the status-evidence constraint — traced to 8 DB-gated test fixture files writing `status: "COMPLETE"` directly, bypassing the real atomic `verifyProcess` write path, not a production defect; re-ran against a from-scratch DB built from exactly this branch's migration set — zero violating rows in every group. Fixed all 8 fixture files (stamping `actualFinish` alongside `status`, matching the real service's atomicity) rather than relax any constraint. Full `pnpm test:db`: 1141/1142 (1 pre-existing unrelated hold-point flake this week's other sessions already log repeatedly).

**Session 19 (AUD-032) — `assertKitReady`'s wrong quantity basis on both sides, PR #62.** The gate combined two independent defects: `required` used `explodeBomItem(item, unitCount, ...)` with the *equipment's* total unit count, demanding a multi-unit job's entire kit be on hand to start work on unit 1 (blocking normal staged material delivery); `available` came from the display-only `computeAvailableForShortage`, which never nets `ISSUE`, so a gate built on it stayed passable forever once ever satisfied. Fixed both: `explodeBomItem` now called with a literal `1` (confirmed correct by re-reading its chain-walk — no `qtyPer` in the schema is equipment-relative, `unitCount` only scales the finished per-unit total once at the end), and new `computeAvailableToIssue` (alongside the untouched, still-display-only `computeAvailableForShortage`) nets `ISSUE`+`SCRAP` and adds back `RETURN`, confirmed against `stock.service.ts`'s `returnStock` as the only `RETURN` writer. One pre-existing test had pinned the old toothless-gate behavior and was rewritten, not just left alone, with a comment explaining the supersession. Flagged as a follow-up, not fixed here: `bom.read.ts`'s display availability numbers have the same ISSUE-blindness. `pnpm test:db` scoped to the two touched files: 83/83.

**Session 20 (AUD-035) — a duration override destructively overwrote the printed envelope, PR #63.** `applyDurationOverride`'s restamp loop wrote the recomputed CPM's MAX offsets back into `JobProcess`'s printed Layer-1 envelope columns on every override — a destructive edit to invariant #10, added by a prior session as an already-reasoned partial mitigation for a different desync, deliberately leaving MIN untouched to avoid corrupting `requiredMinDays`. Traced every reader of the four envelope columns via grep before choosing a design, per the brief's own instruction not to assume: outside the restamp loop itself, the only production reader is `generateSchedule`, checking the job's *originally committed* order window at intake/regeneration time — nothing reads these columns back for a "current CPM position" question. Concluded restamping was never the right fix and **removed it entirely** rather than extending it to also restamp MIN; `applyDurationOverride` now writes only `durationOverrideDays`/`overrideReason` (the override's own input), while `ProcessPlan.plannedStart`/`plannedFinish` (Layer 2) already serves anything wanting the override-adjusted position. `checkFeasibility`'s bare `Error` on the min>max guard hardened into a real `AppError`. `pnpm test:db` scoped to the two touched files: 17/17.

**Merging hit two real conflicts, both in `LEDGER.md`, both resolved by keeping both rows.** #62 merged clean first. #63 conflicted with #62's own new `LEDGER.md` row (both appended near the same spot) — merged `origin/main` into the branch, kept both rows in landing order, re-verified `migrate diff`/typecheck, re-ran CI clean, merged. #64 hit the identical shape of conflict against both #62 and #63's rows — same resolution, same re-verification, same clean CI rerun, merged. **#64 also needed the `migration`-label CI gate resolved**: label wasn't attached before the triggering push, so the check failed reading a stale (label-less) event payload; adding the label after the fact didn't retrigger it — `gh run rerun` reuses the same stale payload — an empty commit was needed to force a fresh `pull_request` event, matching this same gotcha's precedent from PR #61's session. All three PRs (#62, #63, #64) merged to `main`.

**Not applied to production**: #64 carries a real migration (`20260910100000_aud006_check_constraints`) and joins the existing batched rehearse-then-apply-by-hand queue — not something to apply silently as a side effect of merging, per this project's standing `MERGE-RUNBOOK.md` discipline.

**Next**: `docs/mos-execution/LEDGER.md` and `audit/19_MASTER_ISSUE_REGISTER.md` updated for all three sessions in their respective PRs. `audit/SESSION-21` onward (AUD-072/073, AUD-025, AUD-079) still untracked, still needs Swayam's go-ahead before dispatching. AUD-006's migration is now part of the next production migration window.

## Session — Sessions 21–23 (AUD-072/073, AUD-025, AUD-079) dispatched, one closed partial, 10 Sep 2026

Picked up `audit/SESSION-21-AUD-072-073.md`, `SESSION-22-AUD-025.md`, `SESSION-23-AUD-079.md` — three independent findings, dispatched as three parallel background agents, each in its own worktree branched off `origin/main`.

**Session 21 (AUD-072/073) — quality-gate concurrency, PR #67, AUD-072 closed *partial*, not fully.** AUD-073 (`publishSnapshot` could silently revert a VERIFIED snapshot back to PUBLISHED on a race) fixed cleanly — same status-guarded `updateMany`+`create`-fallback pattern `verifySnapshot`/`rejectSnapshot` already used, now throwing `SNAPSHOT_ALREADY_VERIFIED` on a lost race instead of overwriting. AUD-072 (quality gates read-then-act with no lock): added `isolationLevel`/`withSerializationRetry` to `db.ts`, wired `RepeatableRead`+retry into the 5 real call sites (`verifyProcess`, `addUnitToBatch`/`approveDispatchRelease`/`recordDispatch`, `assignUnitToPackage`) — these call `withTenant` directly, not `withJob` as the brief assumed, confirmed by grep before wiring. Live-confirmed the actual Prisma 6.19.3 `40001` error shape (not guessed): `P2034` for a normal write conflict, `P2010`+`meta.code==="40001"` for a raw-query conflict. **Important honest finding, not overclaimed**: a live two-transaction reproduction proved `RepeatableRead` (and even `Serializable`) does *not* close the exact cross-table race the audit describes (verify reads "no open NCR" clean while a concurrent reject opens one) — it's a legal serial history under Postgres MVCC with only a one-directional read-write dependency, so no isolation level aborts it. What the fix does close, and verified: the same-row race (two concurrent verifies of one row) now fails fast and retries instead of silently continuing under READ COMMITTED. Real closure of the cross-table race needs pessimistic locking shared with the NCR/hold-point writers — flagged as follow-up in the PR, a code comment on `verifyProcess`, the ledger, and the register (AUD-072 marked "CLOSED (partial — see caveat)", not fully closed). Tests: 641/641 pure, DB-gated 22/22+43/43 plus one pre-existing unrelated seed flake.

**Session 22 (AUD-025) — `/board`/`/profile` client-visibility bypass, PR #65, fully fixed.** Confirmed `/portal` lives entirely outside the `(app)` route group (same as `/login`) — no redirect-loop risk before shipping the layout-level guard. Confirmed via grep against the full page listing that no third page beyond `board`/`profile` was missing the `clientId !== null` check. Fix landed in three places: `(app)/layout.tsx`'s new guard before the `Promise.all` that was loading `loadJobs` internal telemetry for every actor including clients (the structural fix — blocks the leak at its source, not just at the two named pages), plus the two per-page guards matching the other ten sibling pages. `e2e/auth.spec.ts` extended with 4 scenarios. Lint/typecheck/`pnpm test` clean (637/637). **Gap flagged honestly, not hidden**: `pnpm e2e` itself wasn't run — the isolated worktree had no seeded test DB and the agent declined to spin one up ad hoc rather than risk touching `despl_demo`; PR asks the reviewer to run it against a properly seeded `despl_test` before merging.

**Session 23 (AUD-079) — admin password reset defeats maker–checker, PR #66, fully fixed.** Followed the two-actor precedent from AUD-003's QCP waiver flow, not a same-call `approverUserId` field (which the brief explicitly called out as not closing the gap). `resetUserPassword` no longer immediately resets a `QC`/`ADMIN` target — it records a new tenant-RLS'd `PendingPasswordReset` row (no plaintext ever generated at request time) and returns `{ pending: true, requestId }`; a new `approvePasswordReset` (ADMIN-only, refuses same-actor with `MAKER_CHECKER_VIOLATION`) does the actual reset, reusing `generateTempPassword`/`hashPassword` unchanged. Non-QC/ADMIN targets (SUPERVISOR, PRODUCTION_HEAD, MANAGEMENT, CLIENT_VIEWER) are byte-for-byte unaffected. Added the detective control (other admins notified on any QC/ADMIN-targeted reset) and corrected `assertMakerChecker`'s doc comment per the audit's own note that it "reads as stronger than the system can deliver." **Real operational constraint surfaced and flagged, not silently shipped**: DESPL's own seed provisions exactly one `ADMIN` account — as shipped, this tenant currently has nobody to approve a QC/ADMIN reset request, so a second real admin account needs provisioning before this control is usable. Scoped `pnpm test:db` (`admin.service.test.ts`+`rls-coverage.test.ts`): 38/38, covering all 7 brief scenarios; full-suite run hit 21 unrelated pre-existing environmental timeouts against the shared, bloated `despl_test`, none in touched files. `npx prisma migrate diff --exit-code` clean after fixing the hand-written migration's `approved_by` FK to the schema's established `ON DELETE SET NULL` nullable-actor convention.
...
## Session — PR #61 conflict resolved, 10 Sep 2026

Picked up the standing "PR #61 needs a rebase" item. `git merge origin/main` into `fix/aud-078-qcp-library-tenant-isolation` (not a rebase — matching this repo's existing conflict-resolution precedent from the #62/#63/#64 batch). Three files touched on both sides: `audit/19_MASTER_ISSUE_REGISTER.md` and `src/lib/services/dispatch.service.test.ts` auto-merged clean (disjoint lines — AUD-006's `actualFinish` fixture stamp landed at a different spot than this branch's `tenantId` fixture additions). Only real conflict was `docs/mos-execution/LEDGER.md`, same tail-of-file append-collision shape every parallel-session batch this week has hit — resolved by keeping both sides' rows in landing order (AUD-078's own row, then Sessions 18-20's AUD-006/032/035 rows).

**Verified before committing the merge, not assumed clean.** `pnpm typecheck` clean. `pnpm lint` showed 40 errors/60 warnings, all traced to stray `.claude/worktrees/agent-*/design/support.js` files (`ReactDOM.render`/`no-assign-module-variable` from a bundled asset, not source) — the same pre-existing contamination this file's own AUD-006/AUD-028-059 rows already document; zero errors under `src/`. `pnpm test` (unscoped) hung picking up 2018 `.test.ts` files instead of the real 101 — `.claude/worktrees` now holds enough agent worktrees that vitest's `exclude: ['worktrees']` config entry isn't matching the nested `.claude/worktrees/...` path shape; killed it and ran `vitest run --root src --exclude '**/.claude/**'` directly instead: **633/633 passed**, matching this branch's own pre-merge count. Did not attempt to fix the vitest exclude glob or the lint contamination — both out of scope for a conflict-resolution session, flagged here for whoever picks up repo hygiene next.

Pushed; PR #61 flipped from `CONFLICTING` to `MERGEABLE`, CI re-triggered. Both `migration-pr` and `ci` passed. **PR #61 is now CI-green and mergeable, still unmerged** — per this repo's standing "Swayam merges himself" rule (same block this PR's own session hit earlier this week even after explicit "yes merge it now"s). Ready for `gh pr merge 61 --merge --delete-branch` from Swayam's own terminal.

**Swayam merged it himself, same session, `gh pr merge 61 --merge --delete-branch` succeeded** — the harness's classifier that blocked this exact command earlier in the week didn't block it this time when Swayam ran it directly. Fast-forward merge, remote branch deleted. `docs/mos-execution/LEDGER.md`'s AUD-078 row updated to reflect the real merge (was still saying "not yet merged"); the stale "5 migrations queued" tail row (a leftover from before the 9 Sep batched-window session, itself already marked superseded once) corrected to the real current queue: **4 migrations now on `main`, none applied to production** — AUD-006's `20260910100000_aud006_check_constraints` (PR #64) plus PR #61's three AUD-078 migrations (`20260909200000`–`220000`). Per `MERGE-RUNBOOK.md`, these need a rehearse-then-apply-by-hand window before production sees them; the AUD-078 backfill migration's own session flagged that its preflight (`audit_log`/seed-sentinel authorship count for the 3 global library rows) was only ever checked against local `despl_test`, not production — that check must be re-run for real before applying.
