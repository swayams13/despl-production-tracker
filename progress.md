...
**Needs the user's attention before Phase 2 or any live demo** (flagged, not fixed — destructive/out of scope for this session's code work): the local `despl` dev DB has 35 `Organization` rows (34 leaked from a prior DB-gated test run pointed at the wrong DB), breaking `resolveTenantForLogin`'s single-org assumption and blocking every real `/login` attempt on this machine. This means the entire login → interstitial → password-change flow built this session has been proven by DB-gated tests but **never exercised by a human in a real browser**. Needs an explicit decision: clean up the leaked rows, or fix the single-org assumption.
