...
**Git workflow, changed 13 Aug 2026:** new `demo` branch created from `main`. **Push to `demo` first; merge to `main` only after the user verifies and explicitly approves the promotion.** (This session ran on a harness-assigned branch, `claude/progress-status-check-8ttvkj`, per that session's explicit instructions — not `demo` — see its own session-log entry below for why.)
