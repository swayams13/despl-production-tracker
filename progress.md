...
**Status:** 🟢 **Day 2 under way 13 Aug 2026 — `lib/schedule/` built and (manually/mechanically) verified, not yet run in CI.** Foundation (tenancy/RLS/auth/RBAC), all seed data, and the scheduling engine now exist. **Still nothing calls the engine from a UI or persists a `ScheduleRun`** — no `lib/services/`, no visual component set, no department workspaces. **Next: `lib/services/` (start/submit/verify/delay-reason) + the visual component set.**
...
**Verification caveat, important for the next session:** this session's sandbox had no package-registry access at all (`registry.npmjs.org` returned 403 even bypassing the proxy — confirmed via direct `curl`, not just a failed `pnpm install`), so `node_modules` could not be installed and **`pnpm test`/`lint`/`typecheck` were never actually run**. Confidence instead comes from (a) careful code reading, (b) an independent Python re-implementation of the forward/backward CPM pass and the calendar day-counting run against the real seed JSON (zero mismatches across all 36 processes, both directions), and (c) Python's stdlib `datetime` cross-checking every hardcoded date/weekday claim in the test files. That is a real substitute for reading the code twice, but it is **not** the same as a green `pnpm test` run. Do not assume the suite passes — run `pnpm install && pnpm test && pnpm lint && pnpm typecheck` yourself before building anything else on top of `lib/schedule/`, and fix anything that doesn't match before trusting it further.
